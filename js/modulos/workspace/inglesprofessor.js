...
Workspace.InglesProfessor = {
    ...
    handleAction: async function(acao, b) {
        ...
        switch(acao) {
            ...
            case 'remover-item':
                ...
                Ingles.mostrarAvisoLocal('Item removido!', 'success');
                break;
            case 'add-word':
            ...
            case 'add-debate':
                ...
                if(!dbTopic) return Ingles.mostrarAvisoLocal('Preencha o tópico!', 'warning');
            ...
            case 'aprovar-envio':
                ...
                Ingles.mostrarAvisoLocal('Aprovado! 🌊', 'success');
                break;
            case 'rejeitar-envio':
    ...
    carregarRanking: async function() {
        ...
        try {
            ...
            if(!res?.success) { listEl.innerHTML = '<div style="text-align:center;padding:40px;color:#e74c3c;">Erro ao carregar o ranking.</div>'; return; }
            
            const ranking = res.ranking || [];
            if(!ranking.length) { 
                listEl.innerHTML = '<div style="text-align:center;padding:40px;color:#94a3b8;">Nenhum treino registado ainda.</div>'; 
                return; 
            }
    ...
    renderProfessorTab: function(tabId) {
        ...
        if(tabId === 'biblioteca'){
                ...
                <div class="grid-cards">
                    <div class="prof-card">
                        ...
                        <div class="prof-list-scroll">
                            ${state.words.length ? state.words.map(w=>`<div class="prof-list-item"><span><b>${esc(w.word)}</b> <span style="color:#cbd5e1;margin:0 4px">|</span> ${esc(w.translation)}</span><button data-action="remover-item" data-key="words" data-id="${w.id}" class="ws-btn-danger" style="padding:4px 8px;font-size:11px">✕</button></div>`).join('') : '<span style="color:#94a3b8;font-size:12px;">Vazio</span>'}
                        </div>
                    </div>
                    <div class="prof-card">
                        ...
                        <div class="prof-list-scroll">
                            ${state.phrases.length ? state.phrases.map(p=>`<div class="prof-list-item"><span>${esc(p.phrase)}</span><button data-action="remover-item" data-key="phrases" data-id="${p.id}" class="ws-btn-danger" style="padding:4px 8px;font-size:11px">✕</button></div>`).join('') : '<span style="color:#94a3b8;font-size:12px;">Vazia</span>'}
                        </div>
                    </div>
                    <div class="prof-card">
                    ...
                    <div class="prof-card">
                        ...
                        <div class="prof-list-scroll">
                            ${state.wordPickers.length ? state.wordPickers.map(s=>`<div class="prof-list-item"><span><b>${esc(s.text)}</b> <br><small style="color:#10b981;font-weight:700;">Correta: ${esc(s.options[s.correct])}</small></span><button data-action="remover-item" data-key="wordPickers" data-id="${s.id}" class="ws-btn-danger" style="padding:4px 8px;font-size:11px">✕</button></div>`).join('') : '<span style="color:#94a3b8;font-size:12px;">Vazio</span>'}
                        </div>
                    </div>
                    <div class="prof-card">
                        ...
                        <div class="prof-list-scroll">
                            ${state.minimalPairs.length ? state.minimalPairs.map(m=>`<div class="prof-list-item"><span>${esc(m.a)} <b>vs</b> ${esc(m.b)}</span><button data-action="remover-item" data-key="minimalPairs" data-id="${m.id}" class="ws-btn-danger" style="padding:4px 8px;font-size:11px">✕</button></div>`).join('') : '<span style="color:#94a3b8;font-size:12px;">Vazio</span>'}
                        </div>
                    </div>
                    <div class="prof-card">
                        ...
                        <div class="prof-list-scroll">
                            ${state.debates.length ? state.debates.map(d=>`<div class="prof-list-item"><span><b>Tema:</b> ${esc(d.topic)}</span><button data-action="remover-item" data-key="debates" data-id="${d.id}" class="ws-btn-danger" style="padding:4px 8px;font-size:11px">✕</button></div>`).join('') : '<span style="color:#94a3b8;font-size:12px;">Vazio</span>'}
                        </div>
                    </div>
                    <div class="prof-card">
                        ...
                        <div class="prof-list-scroll">
                            ${state.roleplays.length ? state.roleplays.map(r=>`<div class="prof-list-item"><span><b>${esc(r.title)}</b></span><button data-action="remover-item" data-key="roleplays" data-id="${r.id}" class="ws-btn-danger" style="padding:4px 8px;font-size:11px">✕</button></div>`).join('') : '<span style="color:#94a3b8;font-size:12px;">Vazio</span>'}
                        </div>
                    </div>
                    <div class="prof-card">
                        ...
                        <div class="prof-list-scroll">
                            ${state.questions.length ? state.questions.map(q=>`<div class="prof-list-item"><span>${esc(q.text)}</span><button data-action="remover-item" data-key="questions" data-id="${q.id}" class="ws-btn-danger" style="padding:4px 8px;font-size:11px">✕</button></div>`).join('') : '<span style="color:#94a3b8;font-size:12px;">Vazio</span>'}
                        </div>
                    </div>
                </div>`;
        }
            ...
            document.getElementById('tab-envios').innerHTML=`
                ...
                ${pendentes.length===0 ? '<div style="background:#fff; border:1px dashed #cbd5e1; border-radius:16px; padding:40px; text-align:center;"><span style="font-size:40px;">📭</span><h3 style="color:#475569;margin-top:15px;">Nenhum envio pendente.</h3></div>' : 
                pendentes.map(s=>`
                    <div class="prof-card" style="border-left: 4px solid #6366f1; margin-bottom:16px;">
                        <div style="font-size:12px; color:#64748B; margin-bottom:10px;"><b>👤 ${esc(s.student)}</b> no jogo 🎮 ${esc(s.game)}</div>
                        ...
                        <div style="display:flex; flex-wrap:wrap; gap:12px;">
                            <button data-action="aprovar-envio" data-id="${s.id}" class="ws-btn-success">🌟 Aprovar</button>
                ...
                <div class="prof-list-scroll" style="max-height:300px; background:#fff; border:1px solid #e2e8f0; border-radius:16px; padding:15px;">
                    ${state.pool.length === 0 ? '<div style="text-align:center; color:#94a3b8; padding:20px;">A piscina está vazia.</div>' : state.pool.map(p=>`
