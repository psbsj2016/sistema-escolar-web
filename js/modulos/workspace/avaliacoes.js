...
abrirModalAcessos: async (avaliacaoId, destinoId, isSilent = false) => {
 ...
 salvarProvaOnline: async () => {
   ...
   abrirRecebidas: async (restauro = false) => {
    ...
    // ========================================================================
    // 🖍️ LOUSA v7 FINAL - VISUALIZAÇÃO PROFESSOR + BLOQUEIO TOTAL
    // ========================================================================
    _lousaSSE: null,
    ...
    enviarComandoLousaDireto: async (tipo) => {
        ...
        try {
            const res = await Workspace.api('/workspace/sala/workspace-lousa.html/status', 'PUT', payload);
            console.log('[LOUSA] PUT', payload, '=>', res);
    ...
    abrirLousaFullscreen: async () => {
        ...
        if (Workspace.usuario?.id) {
            try { await Workspace.api('/workspace/sala/workspace-lousa.html/aguardando', 'POST', { usuarioId: Workspace.usuario.id }); } catch(e){}
        }
    },
    ...
    abrirLousaProfessor: (turmaId) => {
        ...
        if(iframe){
            // 🚀 CORREÇÃO: Restauramos o .html para o servidor entregar o ficheiro físico!
            const url = `/workspace-lousa.html?role=professor&room=${encodeURIComponent(tid)}`;
            console.log('[LOUSA] iframe professor URL:', url);
            iframe.setAttribute('src', url);
        }
        if(modal){
            modal.style.display = 'flex'; // Mantém o flex para não quebrar o layout
            document.body.style.overflow = 'hidden';
        }
    },
