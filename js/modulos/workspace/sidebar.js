...
Workspace.Sidebar = {
    ...
    enviarTarefa: async () => {
        ...
        if (!fileInput.files || fileInput.files.length === 0) {
            Toast.show("Selecione um ficheiro (PDF, Word ou Imagem) para enviar o seu trabalho.", "warning");
            return;
        }

        const file = fileInput.files[0];
        if (file.size > 10 * 1024 * 1024) return Toast.show("O ficheiro é muito pesado. Tente um ficheiro até 10MB.", "warning");
        ...
        try {
            ...
            if (entregaRes && entregaRes.success) {
                Toast.show("Trabalho entregue com sucesso!", "success");
                Workspace.Sidebar.abrirModalTarefa(eventoId);
            } else throw new Error(entregaRes.error || "Erro ao gravar entrega.");
        } catch (e) {
            console.error(e);
            Toast.show("Erro ao enviar trabalho. Tente novamente.", "error");
        } finally {
    ...
    salvarNovaTarefa: async () => {
        ...
        if(!titulo || !turmaId || !data) return Toast.show("Preencha o Título, a Turma e o Prazo Limite!", "warning");
        ...
        try {
            ...
            if(res && !res.error) {
                Toast.show("Atividade publicada com sucesso nas agendas dos alunos!", "success");
                ...
                Toast.show(res.error || "Erro ao criar tarefa.", "error");
            }
        } catch(e) { 
            Toast.show("Erro de comunicação.", "error"); 
        } finally { 
            btn.innerText = txt; 
            btn.disabled = false; 
        }
    ...
    apagarTarefa: async (id) => {
        // Mantemos o `confirm` porque é a defesa nativa do navegador contra cliques acidentais destrutivos
        ...
        try {
            ...
            if(res && res.success) {
                Toast.show("Tarefa removida com sucesso!", "success");
                Workspace.Sidebar.abrirPainelTarefasRecebidas(); 
            } else {
                Toast.show("Erro ao apagar tarefa.", "error");
            }
        } catch(e) { 
    ...
    salvarEdicaoTarefa: async (id) => {
        ...
        try {
            ...
            if(res && res.success) {
                Toast.show("Instruções atualizadas com sucesso!", "success");
                const evento = Workspace.Sidebar.tarefasCache.find(e => e.id === id);
                if(evento) evento.descricao = novoTexto;
                Workspace.Sidebar.abrirModalTarefa(id);
            } else {
                Toast.show("Erro ao atualizar instruções.", "error");
            }
        } catch(e) { 
            Toast.show("Erro de comunicação com o servidor.", "error"); 
