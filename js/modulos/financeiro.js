// =========================================================
// MÓDULO FINANCEIRO V111 (QR CODE NATIVO E VALIDAÇÃO DE FATURA)
// =========================================================
...
App.gerarCarnes = async () => {
    ...
    // 🛡️ CERCA DE VALIDAÇÃO: Bloqueia valores nulos ou negativos
...
App.abrirCarneExistente = async (idLote) => {
    const div = document.getElementById('app-content'); 
    div.innerHTML = '<p style="text-align:center; padding:20px;">Gerando Layout de Impressão...</p>';
    ...
    try {
            ...
            <div class="carne-row">
                ...
                <div class="carne-recibo">
                    ...
                    <div class="carne-footer">
                        ...
                        <div class="carne-qr" id="qr-${p.id}">${!hasPix && !escola.qrCodeImagem ? '<div style="font-size:8px; display:flex; height:100%; align-items:center; justify-content:center;">Sem PIX</div>' : ''}</div>
                    </div>
                </div>
            </div>`; 
        }).join('');
        ...
        // 🛡️ A MÁGICA ACONTECE AQUI: Desenha os QR Codes LOCALMENTE via JavaScript
        if (escola.qrCodeImagem) {
            parcelas.forEach(p => {
                const el = document.getElementById(`qr-${p.id}`);
                if(el) el.innerHTML = `<img src="${escola.qrCodeImagem}" style="width:120px; height:120px; object-fit:contain;">`;
            });
        } else if (hasPix) {
            parcelas.forEach(p => {
                const el = document.getElementById(`qr-${p.id}`);
