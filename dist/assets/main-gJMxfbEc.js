...
Qualquer dúvida, estamos à inteira disposição. Tenha um excelente dia! 🌟`,s=`https://api.whatsapp.com/send?phone=${a}&text=${encodeURIComponent(o)}`;window.open(s,`_blank`)}catch(e){console.error(`Erro ao gerar link do WhatsApp:`,e),T.showToast(`Erro ao processar a mensagem.`,`error`)}},window.App=window.App||{};var E=window.App,D={Evento:{bg:`#2ecc71`,text:`#fff`},Feriado:{bg:`#e74c3c`,text:`#fff`},Prova:{bg:`#3498db`,text:`#fff`},Reunião:{bg:`#f39c12`,text:`#fff`}},O=(e,t,n=`text`,r=``,i=``)=>`
    ...
    `).join(``)}},E.editarLancamentoChamada=(e,t,n)=>{document.getElementById(`chamada-aluno`).value=e,document.getElementById(`chamada-turma`).value=``,document.getElementById(`chamada-duracao`).value=n,E.datasLancamentoChamada=[t],E.atualizarFilaDatasUI(),document.getElementById(`btn-cancel-chamada`).style.display=`inline-block`,E.carregarListaChamada(),document.querySelector(`.card`).scrollIntoView({behavior:`smooth`})},E.cancelarEdicaoChamada=()=>{document.getElementById(`chamada-aluno`).value=``,document.getElementById(`btn-cancel-chamada`).style.display=`none`,document.getElementById(`area-lista-chamada`).innerHTML=``,E.datasLancamentoChamada=[new Date().toISOString().split(`T`)[0]],E.atualizarFilaDatasUI(),E.showToast(`Modo de edição cancelado.`,`info`)},E.marcarTodosChamadaGlobal=e=>{let t=document.querySelectorAll(`.status-chamada`);t.length!==0&&(t.forEach(t=>{t.value=e,e===`Falta`?t.style.color=`#e74c3c`:e===`Reposição`?t.style.color=`#f39c12`:e===`Presença`?t.style.color=`#27ae60`:t.style.color=`#333`}),E.showToast(`Lote aplicado na matriz inteira: ${e}!`,`info`))},E.carregarListaChamada=async()=>{let e=document.getElementById(`chamada-turma`).value,t=document.getElementById(`chamada-aluno`).value;if(!e&&!t)return E.showToast(`Selecione uma Turma OU um Aluno específico.`,`warning`);if(E.datasLancamentoChamada.length===0)return E.showToast(`Adicione pelo menos uma data na fila.`,`warning`);let n=document.getElementById(`area-lista-chamada`);n.innerHTML=`<p style="text-align:center; padding:20px;">A preparar Matriz de Classe... ⚡</p>`;try{let r=E.cachePedagogico.alunos,i=E.cachePedagogico.chamadas,a=E.cachePedagogico.planejamentos.filter(e=>e.status!==`Arquivado`).map(e=>e.idAluno),o=t?r.filter(e=>e.id===t):r.filter(t=>t.turma===e);if(o=o.filter(e=>(!e.status||e.status===`Ativo`)&&a.includes(e.id)),o.length===0){n.innerHTML=`<div class="card"><p style="text-align:center; color:#999; margin:0;">Nenhum aluno com Planejamento Ativo foi encontrado para este filtro.</p></div>`;return}let s=`<tr><th style="padding:15px; text-align:left; position:sticky; left:0; background:#f8f9fa; z-index:2; border-right:1px solid #ddd;">NOME DO ALUNO</th>`;E.datasLancamentoChamada.forEach(e=>{s+=`<th style="padding:15px; text-align:center; min-width:140px;">${e.split(`-`).reverse().join(`/`)}</th>`}),s+=`</tr>`;let c=``;o.forEach(e=>{c+=`<tr style="border-bottom:1px solid #eee;" class="linha-aluno-matriz" data-id="${e.id}" data-nome="${E.escapeHTML(e.nome)}">`,c+=`<td style="padding:12px; font-weight:500; position:sticky; left:0; background:#fff; z-index:1; border-right:1px solid #ddd;">${E.escapeHTML(e.nome)}</td>`,E.datasLancamentoChamada.forEach(t=>{let n=i.find(n=>String(n.idAluno)===String(e.id)&&n.data===t),r=n?n.status:`Presença`,a=n?`data-id-chamada="${n.id}"`:``;c+=`
                <td style="padding:8px;">
                    <select class="status-chamada" data-data="${t}" ${a} style="width:100%; padding:8px; border-radius:5px; border:1px solid #ccc; font-weight:bold; color:${r===`Falta`?`#e74c3c`:r===`Reposição`?`#f39c12`:`#27ae60`};" onchange="this.style.color = this.value==='Falta'?'#e74c3c': (this.value==='Reposição'?'#f39c12':'#27ae60')">
        ...
        `}catch{n.innerHTML=`<p style="color:red; text-align:center;">Erro ao processar a matriz.</p>`}},E.salvarChamadaLote=async()=>{let e=document.getElementById(`chamada-duracao`).value||`01:00`,t=document.querySelectorAll(`.linha-aluno-matriz`);if(t.length===0)return;let n=document.querySelector(`button[onclick="App.salvarChamadaLote()"]`),r=n.innerText;n.innerText=`A arquivar matriz em lote... ⚡`,n.disabled=!0,document.body.style.cursor=`wait`;try{let i=E.cachePedagogico.planejamentos,a=E.cachePedagogico.chamadas,o=[],s=new Set,c=[],l=[...a];if(t.forEach(e=>{let t=e.getAttribute(`data-id`),n=e.getAttribute(`data-nome`);i.some(e=>e.idAluno===t&&e.status!==`Arquivado`)||o.push(n)}),o.length>0){E.showToast(`Bloqueado: Crie um Planejamento para: ${o.join(`, `)}`,`error`),n&&(n.innerText=r,n.disabled=!1),document.body.style.cursor=`default`;return}t.forEach(t=>{let n=t.getAttribute(`data-id`),r=t.getAttribute(`data-nome`);t.querySelectorAll(`.status-chamada`).forEach(t=>{let i=t.value;if(i===`N/A`)return;let o=t.getAttribute(`data-data`),u=t.getAttribute(`data-id-chamada`);s.add(n);let d=null;d=u?a.find(e=>String(e.id)===String(u)):a.find(e=>String(e.idAluno)===String(n)&&e.data===o);let f={idAluno:n,nomeAluno:r,data:o,status:i,duracao:e};if(d){let t={...d,data:o,status:i,duracao:e};c.push(E.api(`/chamadas/${d.id}`,`PUT`,t));let n=l.findIndex(e=>String(e.id)===String(d.id));n!==-1&&(l[n]=t)}else f.id=typeof crypto.randomUUID==`function`?crypto.randomUUID():Date.now().toString(36)+Math.random().toString(36).substr(2),c.push(E.api(`/chamadas`,`POST`,f)),l.push(f)})}),await Promise.all(c);let u=``;try{let e=[];s.forEach(t=>{let n=i.find(e=>e.idAluno===t&&e.status!==`Arquivado`);n&&typeof E.processarAutoAjustePlano==`function`&&(n=E.processarAutoAjustePlano(n,l),e.push(E.api(`/planejamentos/${n.id}`,`PUT`,n)))}),e.length>0&&(await Promise.all(e),u=` e Planos Auto-Ajustados!`)}catch(e){console.log(`Aviso: Falha no auto-ajuste de fundo.`,e)}E.showToast(`${c.length} registos processados com sucesso${u}`,`success`),E.cancelarEdicaoChamada();let d=document.getElementById(`app-content`);d&&(d.innerHTML=`<p style="text-align:center; color:#666; padding:20px;">A atualizar a tabela de histórico... ⏳</p>`),await E.renderizarChamadaPro()}catch{E.showToast(`Erro ao guardar a matriz de chamadas.`,`error`)}finally{n&&(n.innerText=r,n.disabled=!1),document.body.style.cursor=`default`}},E.excluirLancamentoChamada=e=>{E.confirmar(`Excluir Frequência`,`Tem a certeza que deseja excluir esta chamada do sistema?`,`Excluir`,`#e74c3c`,async()=>{await E.api(`/chamadas/${e}`,`DELETE`);let t=document.getElementById(`app-content`);t&&(t.innerHTML=`<p style="text-align:center; color:#666; padding:20px;">Atualizando frequência... ⏳</p>`),await E.renderizarChamadaPro()})},E.renderizarCalendarioPro=async()=>{E.setTitulo(`Calendário`);let e=document.getElementById(`app-content`);e.innerHTML=`A carregar calendário...`,E.calendarState||={month:new Date().getMonth(),year:new Date().getFullYear()};try{let t=await E.api(`/eventos?_t=${Date.now()}`),n=[`Janeiro`,`Fevereiro`,`Março`,`Abril`,`Maio`,`Junho`,`Julho`,`Agosto`,`Setembro`,`Outubro`,`Novembro`,`Dezembro`][E.calendarState.month],r=`
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
