...
`;A.renderizarSelecaoRelatorio=async()=>{let e=document.getElementById(`app-content`);e.innerHTML=`<p style="text-align:center; padding:20px; color:#666;">A carregar períodos disponíveis... ⏳</p>`;try{let t=await A.api(`/financeiro`),n=new Set,r=new Date().getFullYear();n.add(r),n.add(r+1),t.forEach(e=>{if(e.vencimento){let t=parseInt(e.vencimento.split(`-`)[0]);isNaN(t)||n.add(t)}if(e.dataPagamento){let t=parseInt(e.dataPagamento.split(`-`)[0]);isNaN(t)||n.add(t)}});let i=Array.from(n).sort((e,t)=>t-e).map(e=>`<option value="${e}" ${e===r?`selected`:``}>${e}</option>`).join(``),a=[`Janeiro`,`Fevereiro`,`Março`,`Abril`,`Maio`,`Junho`,`Julho`,`Agosto`,`Setembro`,`Outubro`,`Novembro`,`Dezembro`].map((e,t)=>`<option value="${t+1}" ${t===new Date().getMonth()?`selected`:``}>${e}</option>`).join(``),o=`
    ...
    `;e.innerHTML=A.UI.card(``,``,i,`100%`)},A.renderizarMenuDocumentosOficiais=async()=>{let e=document.getElementById(`app-content`);e.innerHTML=`<p style="text-align:center; padding:20px; color:#666;">A carregar base de alunos... ⏳</p>`;try{let t=(await A.api(`/alunos`)).filter(e=>!e.status||e.status===`Ativo`);e.innerHTML=`
        ...
        `}catch{e.innerHTML=`<p>Erro ao carregar dados.</p>`}},A.gerarDocumentoOficialPrint=async()=>{let e=document.getElementById(`doc-aluno-oficial`).value,t=document.getElementById(`doc-tipo-oficial`).value,n=new Date,r=`${n.getDate()} de ${[`janeiro`,`fevereiro`,`março`,`abril`,`maio`,`junho`,`julho`,`agosto`,`setembro`,`outubro`,`novembro`,`dezembro`][n.getMonth()]} de ${n.getFullYear()}`,i=n.toLocaleDateString(`pt-BR`);if(!e)return A.showToast(`Selecione um aluno na lista.`,`warning`);let a=document.querySelector(`button[onclick="App.gerarDocumentoOficialPrint()"]`),o=a.innerText;a.innerText=`A Processar... ⏳`,a.disabled=!0,document.body.style.cursor=`wait`;try{let n=(await A.api(`/alunos`)).find(t=>t.id===e)||{},a=await A.api(`/escola`)||{nome:`A INSTITUIÇÃO`,cnpj:`00.000.000/0000-00`},o=a.endereco?`${a.endereco}, ${a.numero||`S/N`} - ${a.bairro||``}. ${a.cidade||``}-${a.estado||``} | CEP: ${a.cep||``}`:``,s=`Local não informado`;a.cidade&&a.estado?s=`${A.escapeHTML(a.cidade)} - ${A.escapeHTML(a.estado)}`:a.cidade&&(s=A.escapeHTML(a.cidade));let c=`${s}, ${r}.`,l=document.getElementById(`doc-area-oficial`);l.innerHTML=`<p style="text-align:center;">Gerando Layout... ⏳</p>`;let u=`
        ...
        `;if(t===`contrato`){let e=parseFloat(n.valorMensalidade||0).toLocaleString(`pt-BR`,{minimumFractionDigits:2});l.innerHTML=`
                ...
                <div class="print-sheet" style="font-family: Arial, sans-serif; color: #000; line-height: 1.6;">
                    ...
                    <p style="text-align: justify; margin-top:0; font-size:14px;">O presente contrato tem como objeto a prestação de serviços educacionais por parte da CONTRATADA ao CONTRATANTE, referente ao curso de <b>${A.escapeHTML(n.curso||`Não especificado`)}</b>, a ser ministrado na turma <b>${A.escapeHTML(n.turma||`Não especificada`)}</b>.</p>
                    ...
                    <p style="text-align: right; margin-top: 50px; font-size:14px;">${c}</p>
            ...
            `;let e=document.createElement(`style`);e.innerHTML=`@media print { @page { size: A4 portrait; margin: 15mm; } }`,l.appendChild(e)}}catch{A.showToast(`Erro ao gerar o documento.`,`error`)}finally{a.innerText=o,a.disabled=!1,document.body.style.cursor=`default`}},A.renderizarMenuCertificados=async()=>{let e=document.getElementById(`app-content`);e.innerHTML=`<p style="text-align:center; padding:20px; color:#666;">A preparar o ambiente de graduação... 🎓</p>`;try{let t=(await A.api(`/alunos`)).filter(e=>!e.status||e.status===`Ativo`);e.innerHTML=`
