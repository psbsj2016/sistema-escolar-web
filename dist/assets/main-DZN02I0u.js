...
`;A.renderizarSelecaoRelatorio=async()=>{let e=document.getElementById(`app-content`);e.innerHTML=`<p style="text-align:center; padding:20px; color:#666;">A carregar períodos disponíveis... ⏳</p>`;try{let t=await A.api(`/financeiro`),n=new Set,r=new Date().getFullYear();n.add(r),n.add(r+1),t.forEach(e=>{if(e.vencimento){let t=parseInt(e.vencimento.split(`-`)[0]);isNaN(t)||n.add(t)}if(e.dataPagamento){let t=parseInt(e.dataPagamento.split(`-`)[0]);isNaN(t)||n.add(t)}});let i=Array.from(n).sort((e,t)=>t-e).map(e=>`<option value="${e}" ${e===r?`selected`:``}>${e}</option>`).join(``),a=[`Janeiro`,`Fevereiro`,`Março`,`Abril`,`Maio`,`Junho`,`Julho`,`Agosto`,`Setembro`,`Outubro`,`Novembro`,`Dezembro`].map((e,t)=>`<option value="${t+1}" ${t===new Date().getMonth()?`selected`:``}>${e}</option>`).join(``),o=`
    ...
    `;e.innerHTML=A.UI.card(``,``,r,`500px`)},A.gerarDossie=async()=>{let e=document.getElementById(`dossie-ano`).value,t=parseInt(document.getElementById(`dossie-mes`).value),n=[`Janeiro`,`Fevereiro`,`Março`,`Abril`,`Maio`,`Junho`,`Julho`,`Agosto`,`Setembro`,`Outubro`,`Novembro`,`Dezembro`],r=n[t-1],i=t-1,a=parseInt(e);i===0&&(i=12,--a);let o=n[i-1],s=document.getElementById(`app-content`);s.innerHTML=`<p style="text-align:center; padding:20px; font-size:14px;">Processando Dashboard Corporativo... ⏳</p>`,document.body.style.cursor=`wait`;try{let[n,c,l,u,d]=await Promise.all([A.api(`/alunos`),A.api(`/turmas`),A.api(`/cursos`),A.api(`/financeiro`),A.api(`/escola`)]),f=new Date,p=e=>parseFloat(e||0).toLocaleString(`pt-BR`,{style:`currency`,currency:`BRL`}),m=e=>parseFloat(e.valorPago1||e.valor)+parseFloat(e.valorPago2||0),h=e=>e.descricao&&e.descricao.toLowerCase().includes(`venda`)||e.idCarne&&e.idCarne.includes(`VENDA`),g=u.filter(t=>t.vencimento&&t.vencimento.startsWith(e)&&t.tipo===`Receita`),_=g.filter(e=>parseInt(e.vencimento.split(`-`)[1])===t),v=g.filter(e=>e.status===`Pago`).reduce((e,t)=>e+m(t),0),y=g.filter(e=>!h(e)).reduce((e,t)=>e+(parseFloat(t.valor)||0),0),b=_.filter(e=>!h(e)).reduce((e,t)=>e+(parseFloat(t.valor)||0),0),x=u.filter(e=>e.status===`Pendente`&&e.tipo===`Receita`&&new Date(e.vencimento+`T00:00:00`)<f).reduce((e,t)=>e+(parseFloat(t.valor)||0),0),S=_.filter(e=>e.status===`Pago`&&!h(e)).reduce((e,t)=>e+m(t),0),C=_.filter(e=>e.status===`Pago`&&h(e)).reduce((e,t)=>e+m(t),0),w=S+C,T={},E={};_.filter(e=>e.status===`Pago`).forEach(e=>{let t=h(e)?E:T,n=e.formaPagamento||`Outros`,r=e.formaPagamento2;t[n]=(t[n]||0)+parseFloat(e.valorPago1||e.valor),r&&(t[r]=(t[r]||0)+parseFloat(e.valorPago2||0))});let D=``,O=0;for(let e=1;e<=t;e++){let t=g.filter(t=>parseInt(t.vencimento.split(`-`)[1])===e&&t.status===`Pago`),n=t.filter(e=>!h(e)).reduce((e,t)=>e+m(t),0),r=t.filter(e=>h(e)).reduce((e,t)=>e+m(t),0),i=n+r;O+=i,D+=`<tr style="border-bottom:1px solid #f1f5f9;">
                <td style="padding:6px; font-weight:bold; color:#475569;">${[`Jan`,`Fev`,`Mar`,`Abr`,`Mai`,`Jun`,`Jul`,`Ago`,`Set`,`Out`,`Nov`,`Dez`][e-1]}</td>
                <td style="padding:6px; text-align:right; color:#2563eb;">${p(n)}</td>
                <td style="padding:6px; text-align:right; color:#7c3aed;">${p(r)}</td>
                <td style="padding:6px; text-align:right; font-weight:bold; color:#0f172a;">${p(i)}</td>
            </tr>`}let k={};c.forEach(e=>k[e.nome]=0);let j={};l.forEach(e=>j[e.nome]=0);let M={ativo:{total:0,masc:0,fem:0,color:`#16a34a`,label:`Ativos`},trancado:{total:0,masc:0,fem:0,color:`#d97706`,label:`Trancados`},cancelado:{total:0,masc:0,fem:0,color:`#ea580c`,label:`Cancelados`},excluido:{total:0,masc:0,fem:0,color:`#dc2626`,label:`Excluídos`}},P={};n.forEach(e=>{e.turma&&k[e.turma]!==void 0&&k[e.turma]++,e.curso&&j[e.curso]!==void 0&&j[e.curso]++;let t=(e.status||`ativo`).toLowerCase().trim();t=t.includes(`exclu`)?`excluido`:t.includes(`cancel`)?`cancelado`:t.includes(`tranc`)?`trancado`:`ativo`,M[t]&&(M[t].total++,e.sexo===`Masculino`?M[t].masc++:e.sexo===`Feminino`&&M[t].fem++);let n=(e.pais||e.nacionalidade||`Brasil`).trim().toUpperCase();n===``&&(n=`BRASIL`),P[n]=(P[n]||0)+1});let F=`<div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(130px, 1fr)); gap:10px; margin-bottom:15px;">`;Object.keys(M).forEach(e=>{let t=M[e],n=t.total>0?Math.round(t.masc/t.total*100):0,r=t.total>0?Math.round(t.fem/t.total*100):0;F+=`
                <div style="border-left: 3px solid ${t.color}; background:#f8fafc; padding:8px 10px; border-radius:4px; box-shadow: 0 1px 2px rgba(0,0,0,0.02);">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:4px;">
                        <strong style="font-size:10px; text-transform:uppercase; color:#64748b;">${t.label}</strong>
                        <span style="font-size:15px; font-weight:bold; color:${t.color};">${t.total}</span>
                    </div>
            ...
            `}),F+=`</div>`;let I=e=>{let t=Object.keys(e).filter(t=>e[t]>0);return t.length===0?`<div style="font-size:11px; color:#94a3b8; padding:5px;">Nenhum registo ativo.</div>`:`<div style="display:flex; flex-wrap:wrap; gap:6px;">`+t.map(t=>`<div style="background:#f1f5f9; border:1px solid #e2e8f0; padding:4px 8px; border-radius:4px; display:flex; gap:8px; align-items:center; font-size:10px;">
                    <span style="color:#334155; font-weight:500;">${A.escapeHTML(t)}</span>
                    <strong style="background:#cbd5e1; color:#0f172a; padding:1px 5px; border-radius:10px; font-size:9px;">${e[t]}</strong>
                </div>`).join(``)+`</div>`},L=u.filter(n=>n.status===`Pendente`&&n.tipo===`Receita`&&n.vencimento&&new Date(n.vencimento+`T00:00:00`).getFullYear()===parseInt(e)&&new Date(n.vencimento+`T00:00:00`).getMonth()+1===t).reduce((e,t)=>e+(parseFloat(t.valor)||0),0),R=u.filter(e=>e.status===`Pendente`&&e.tipo===`Receita`&&e.vencimento&&new Date(e.vencimento+`T00:00:00`).getFullYear()===a&&new Date(e.vencimento+`T00:00:00`).getMonth()+1===i).reduce((e,t)=>e+(parseFloat(t.valor)||0),0);s.innerHTML=`
            ${N}
            <style>
            ...
            <div class="print-sheet dashboard-container">
                ...
                <div style="display:flex; justify-content:space-between; align-items:center; border-bottom: 3px solid #1e293b; padding-bottom: 12px; margin-bottom:20px; flex-wrap:wrap; gap:10px;">
                    <div style="display:flex; align-items:center; gap:12px;">
                        ${d.foto?`<img src="${d.foto}" style="height:40px; object-fit:contain;">`:``} 
                        <div>
                            <h2 style="margin:0; text-transform:uppercase; color:#0f172a; font-size:18px; font-weight:900;">${A.escapeHTML(d.nome)}</h2>
                            <div style="font-size:10px; color:#64748b; margin-top:2px;">CNPJ: ${A.escapeHTML(d.cnpj)}</div>
                        </div>
                    </div>
                    <div style="text-align:right;">
                        <div style="font-weight:900; font-size:14px; color:#0f172a; text-transform:uppercase;">Dossiê Executivo - ${r}/${e}</div>
                        <div style="font-size:10px; color:#64748b; margin-top:2px;">Emissão: ${f.toLocaleDateString(`pt-BR`)} às ${f.toLocaleTimeString(`pt-BR`)}</div>
                    </div>
                </div>
                ...
                <div class="grid-2">
                    <div class="exec-box">
                        <h3 class="exec-title">Desempenho Financeiro (${r})</h3>
                        <table class="exec-table" style="margin-bottom:15px;">
                            ...
                            <tr style="background:#f8fafc; border-top:1px solid #e2e8f0;"><td style="font-weight:bold;">Faturamento do Mês:</td><td style="text-align:right; font-size:13px; font-weight:900; color:#16a34a;">${p(w)}</td></tr>
                        </table>
                        ...
                        <div style="display:flex; gap:10px; align-items:stretch; border-top:1px dashed #cbd5e1; padding-top:15px; margin-top:auto;">
                            <div style="flex:1; background:#fff; border: 1px solid #fee2e2; border-radius:4px; padding:10px; text-align:center;">
                                <div style="font-size:9px; color:#ef4444; font-weight:bold; text-transform:uppercase;">Inad. Ant. (${o})</div>
                                <div style="font-size:14px; font-weight:bold; color:#dc2626; margin-top:4px;">${p(R)}</div>
                            </div>
                            <div style="flex:1; background:#fef2f2; border: 1px solid #ef4444; border-radius:4px; padding:10px; text-align:center; box-shadow: inset 0 2px 4px rgba(239,68,68,0.05);">
                                <div style="font-size:10px; color:#991b1b; font-weight:bold; text-transform:uppercase;">Inad. Vigente (${r})</div>
                                <div style="font-size:18px; font-weight:900; color:#991b1b; margin-top:4px;">${p(L)}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="exec-box">
                        ...
                        <table class="exec-table">
                            ...
                            <tbody>${D}</tbody>
                            <tfoot><tr style="background:#f8fafc; border-top:2px solid #cbd5e1;"><td colspan="3" style="text-align:right; font-weight:bold; padding-top:10px;">LÍQUIDO ACUMULADO:</td><td style="text-align:right; font-weight:900; font-size:13px; color:#16a34a; padding-top:10px;">${p(O)}</td></tr></tfoot>
                        </table>
                    </div>
                </div>
            ...
            </div>`,setTimeout(()=>{let e={responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},cutout:`70%`},t=[`#3b82f6`,`#8b5cf6`,`#f59e0b`,`#10b981`,`#f97316`,`#64748b`];Object.keys(T).length>0&&document.getElementById(`grafMensalidade`)&&new Chart(document.getElementById(`grafMensalidade`),{type:`doughnut`,data:{labels:Object.keys(T),datasets:[{data:Object.values(T),backgroundColor:t,borderWidth:0}]},options:e}),Object.keys(E).length>0&&document.getElementById(`grafVenda`)&&new Chart(document.getElementById(`grafVenda`),{type:`doughnut`,data:{labels:Object.keys(E),datasets:[{data:Object.values(E),backgroundColor:t,borderWidth:0}]},options:e}),Object.keys(P).length>0&&document.getElementById(`grafPais`)&&new Chart(document.getElementById(`grafPais`),{type:`bar`,data:{labels:Object.keys(P),datasets:[{data:Object.values(P),backgroundColor:`#475569`,borderRadius:3}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},scales:{x:{grid:{display:!1},ticks:{font:{size:9},maxRotation:0}},y:{beginAtZero:!0,border:{display:!1},ticks:{stepSize:1,font:{size:9}}}}}})},300)}catch{A.showToast(`Erro ao gerar dossiê.`,`error`)}finally{document.body.style.cursor=`default`}},A.gerarFichaSetup=async()=>{A.setTitulo(`Ficha de Matrícula`);let e=document.getElementById(`app-content`);e.innerHTML=`<p style="text-align:center;">Carregando...</p>`;try{let t=`
            <div style="display:flex; gap:10px; align-items:flex-end; flex-wrap:wrap;">
