...
`;A.renderizarSelecaoRelatorio=async()=>{let e=document.getElementById(`app-content`);e.innerHTML=`<p style="text-align:center; padding:20px; color:#666;">A carregar períodos disponíveis... ⏳</p>`;try{let t=await A.api(`/financeiro`),n=new Set,r=new Date().getFullYear();n.add(r),n.add(r+1),t.forEach(e=>{if(e.vencimento){let t=parseInt(e.vencimento.split(`-`)[0]);isNaN(t)||n.add(t)}if(e.dataPagamento){let t=parseInt(e.dataPagamento.split(`-`)[0]);isNaN(t)||n.add(t)}});let i=Array.from(n).sort((e,t)=>t-e).map(e=>`<option value="${e}" ${e===r?`selected`:``}>${e}</option>`).join(``),a=[`Janeiro`,`Fevereiro`,`Março`,`Abril`,`Maio`,`Junho`,`Julho`,`Agosto`,`Setembro`,`Outubro`,`Novembro`,`Dezembro`].map((e,t)=>`<option value="${t+1}" ${t===new Date().getMonth()?`selected`:``}>${e}</option>`).join(``),o=`
    ...
    `;e.innerHTML=A.UI.card(``,``,r,`500px`)},A.gerarDossie=async()=>{let e=document.getElementById(`dossie-ano`).value,t=parseInt(document.getElementById(`dossie-mes`).value),n=[`Janeiro`,`Fevereiro`,`Março`,`Abril`,`Maio`,`Junho`,`Julho`,`Agosto`,`Setembro`,`Outubro`,`Novembro`,`Dezembro`],r=n[t-1],i=t-1,a=parseInt(e);i===0&&(i=12,--a);let o=n[i-1],s=document.getElementById(`app-content`);s.innerHTML=`<p style="text-align:center; padding:20px;">Processando Inteligência de Negócios... ⏳</p>`,document.body.style.cursor=`wait`;try{let[n,c,l,u,d]=await Promise.all([A.api(`/alunos`),A.api(`/turmas`),A.api(`/cursos`),A.api(`/financeiro`),A.api(`/escola`)]),f=new Date,p=e=>parseFloat(e||0).toLocaleString(`pt-BR`,{style:`currency`,currency:`BRL`}),m=e=>parseFloat(e.valorPago1||e.valor)+parseFloat(e.valorPago2||0),h=e=>e.descricao&&e.descricao.toLowerCase().includes(`venda`)||e.idCarne&&e.idCarne.includes(`VENDA`),g=u.filter(t=>t.vencimento&&t.vencimento.startsWith(e)&&t.tipo===`Receita`),_=g.filter(e=>parseInt(e.vencimento.split(`-`)[1])===t),v=g.filter(e=>e.status===`Pago`).reduce((e,t)=>e+m(t),0),y=g.filter(e=>!h(e)).reduce((e,t)=>e+(parseFloat(t.valor)||0),0),b=_.filter(e=>!h(e)).reduce((e,t)=>e+(parseFloat(t.valor)||0),0),x=u.filter(e=>e.status===`Pendente`&&e.tipo===`Receita`&&new Date(e.vencimento+`T00:00:00`)<f).reduce((e,t)=>e+(parseFloat(t.valor)||0),0),S=_.filter(e=>e.status===`Pago`&&!h(e)).reduce((e,t)=>e+m(t),0),C=_.filter(e=>e.status===`Pago`&&h(e)).reduce((e,t)=>e+m(t),0),w=S+C,T={},E={};_.filter(e=>e.status===`Pago`).forEach(e=>{let t=h(e)?E:T,n=e.formaPagamento||`Outros`,r=e.formaPagamento2;t[n]=(t[n]||0)+parseFloat(e.valorPago1||e.valor),r&&(t[r]=(t[r]||0)+parseFloat(e.valorPago2||0))});let D=``,O=0;for(let e=1;e<=t;e++){let t=g.filter(t=>parseInt(t.vencimento.split(`-`)[1])===e&&t.status===`Pago`),n=t.filter(e=>!h(e)).reduce((e,t)=>e+m(t),0),r=t.filter(e=>h(e)).reduce((e,t)=>e+m(t),0),i=n+r;O+=i,D+=`<tr style="border-bottom:1px solid #eee;">
            ...
            `}),F+=`</div>`;let I=`<table class="exec-table"><thead><tr><th>CURSO</th><th style="text-align:right;">ALUNOS</th></tr></thead><tbody>`+(l.length?l.map(e=>`<tr><td>${A.escapeHTML(e.nome)}</td><td style="text-align:right; font-weight:bold;">${j[e.nome]||0}</td></tr>`).join(``):`<tr><td colspan="2">Sem dados</td></tr>`)+`</tbody></table>`,L=`<table class="exec-table"><thead><tr><th>TURMA</th><th style="text-align:right;">ALUNOS</th></tr></thead><tbody>`+(c.length?c.map(e=>`<tr><td>${A.escapeHTML(e.nome)}</td><td style="text-align:right; font-weight:bold;">${k[e.nome]||0}</td></tr>`).join(``):`<tr><td colspan="2">Sem dados</td></tr>`)+`</tbody></table>`,R=u.filter(n=>n.status===`Pendente`&&n.tipo===`Receita`&&n.vencimento&&new Date(n.vencimento+`T00:00:00`).getFullYear()===parseInt(e)&&new Date(n.vencimento+`T00:00:00`).getMonth()+1===t).reduce((e,t)=>e+(parseFloat(t.valor)||0),0),z=u.filter(e=>e.status===`Pendente`&&e.tipo===`Receita`&&e.vencimento&&new Date(e.vencimento+`T00:00:00`).getFullYear()===a&&new Date(e.vencimento+`T00:00:00`).getMonth()+1===i).reduce((e,t)=>e+(parseFloat(t.valor)||0),0);s.innerHTML=`
            ${N}
            <style>
            ...
            <div class="print-sheet" style="font-family: Arial, sans-serif;">
                ...
                <div style="display:flex; justify-content:space-between; align-items:center; border-bottom: 2px solid #2c3e50; padding-bottom: 10px; margin-bottom:15px;">
                    <div style="display:flex; align-items:center; gap:15px;">
                        ${d.foto?`<img src="${d.foto}" style="height:40px; object-fit:contain;">`:``} <div><h2 style="margin:0; text-transform:uppercase; color:#2c3e50; font-size:16px;">${A.escapeHTML(d.nome)}</h2><div style="font-size:10px; color:#666;">CNPJ: ${A.escapeHTML(d.cnpj)}</div></div>
                    </div>
                    <div style="text-align:right;">
                        <div style="font-weight:900; font-size:14px; color:#2c3e50; text-transform:uppercase;">Dossiê Analítico - ${r}/${e}</div>
                        <div style="font-size:9px; color:#666;">Emissão: ${f.toLocaleDateString(`pt-BR`)} às ${f.toLocaleTimeString(`pt-BR`)}</div>
                    </div>
                </div>
                ...
                <div class="exec-grid">
                    <div class="exec-box">
                        <h3 class="exec-title">Desempenho Financeiro (${r}/${e})</h3>
                        <table class="exec-table" style="margin-bottom:10px;">
                            <tr><td>Entradas de Mensalidades:</td><td style="text-align:right; font-weight:bold; color:#2980b9;">${p(S)}</td></tr>
                            <tr><td>Entradas de Vendas/Lojas:</td><td style="text-align:right; font-weight:bold; color:#8e44ad;">${p(C)}</td></tr>
                            <tr style="background:#f4f8f9;"><td style="font-weight:bold;">RECEITA TOTAL ARRECADADA:</td><td style="text-align:right; font-size:14px; font-weight:900; color:#27ae60;">${p(w)}</td></tr>
                        </table>
                    ...
                    <div class="exec-box" style="border-top: 3px solid #e74c3c; background: #fffcfc;">
                        ...
                        <div style="display:flex; gap:10px; align-items:stretch;">
                            <div style="flex:1; background:#fff; border: 1px solid #fadbd8; border-radius:4px; padding:10px; text-align:center; display:flex; flex-direction:column; justify-content:center;">
                                <div style="font-size:10px; color:#e74c3c; font-weight:bold; text-transform:uppercase;">Mês Anterior<br>(${o}/${a})</div>
                                <div style="font-size:18px; font-weight:bold; color:#e74c3c; margin-top:5px;">${p(z)}</div>
                            </div>
                            <div style="flex:1; background:#fdedec; border: 1px solid #e74c3c; border-radius:4px; padding:10px; text-align:center; display:flex; flex-direction:column; justify-content:center; box-shadow: inset 0 2px 5px rgba(231,76,60,0.1);">
                                <div style="font-size:11px; color:#c0392b; font-weight:bold; text-transform:uppercase;">Mês Vigente<br>(${r}/${e})</div>
                                <div style="font-size:24px; font-weight:900; color:#c0392b; margin-top:5px;">${p(R)}</div>
                            </div>
                        </div>
                    </div>

                    <div class="exec-box">
                        <h3 class="exec-title">Histórico de Receitas Brutas (${e})</h3>
                        <table class="exec-table">
                            ...
                            <tbody>${D}</tbody>
                            <tfoot><tr style="background:#f4f8f9;"><td colspan="3" style="text-align:right; font-weight:bold;">ACUMULADO ATÉ ${r.toUpperCase()}:</td><td style="text-align:right; font-weight:900; color:#27ae60;">${p(O)}</td></tr></tfoot>
                        </table>
                    </div>
                </div>
            ...
            </div>`,setTimeout(()=>{let e={responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},cutout:`70%`},t=[`#3498db`,`#9b59b6`,`#f1c40f`,`#2ecc71`,`#e67e22`,`#95a5a6`];Object.keys(T).length>0&&document.getElementById(`grafMensalidade`)&&new Chart(document.getElementById(`grafMensalidade`),{type:`doughnut`,data:{labels:Object.keys(T),datasets:[{data:Object.values(T),backgroundColor:t,borderWidth:0}]},options:e}),Object.keys(E).length>0&&document.getElementById(`grafVenda`)&&new Chart(document.getElementById(`grafVenda`),{type:`doughnut`,data:{labels:Object.keys(E),datasets:[{data:Object.values(E),backgroundColor:t,borderWidth:0}]},options:e}),Object.keys(P).length>0&&document.getElementById(`grafPais`)&&new Chart(document.getElementById(`grafPais`),{type:`bar`,data:{labels:Object.keys(P),datasets:[{data:Object.values(P),backgroundColor:`#34495e`,borderRadius:3}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},scales:{x:{grid:{display:!1},ticks:{font:{size:9}}},y:{beginAtZero:!0,border:{display:!1},ticks:{stepSize:1,font:{size:9}}}}}})},300)}catch{A.showToast(`Erro ao gerar dossiê.`,`error`)}finally{document.body.style.cursor=`default`}},A.gerarFichaSetup=async()=>{A.setTitulo(`Ficha de Matrícula`);let e=document.getElementById(`app-content`);e.innerHTML=`<p style="text-align:center;">Carregando...</p>`;try{let t=`
            <div style="display:flex; gap:10px; align-items:flex-end; flex-wrap:wrap;">
