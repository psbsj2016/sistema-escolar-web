    ...
    `,o.appendChild(s),document.body.appendChild(o),setTimeout(()=>{o.style.opacity=`1`,s.style.transform=`scale(1)`},10);let c=()=>{o.style.opacity=`0`,s.style.transform=`scale(0.9)`,setTimeout(()=>document.body.removeChild(o),200)};document.getElementById(`btn-no-${a}`).onclick=c,document.getElementById(`btn-yes-${a}`).onclick=()=>{c(),i()}},E.filtrarTabela=(e,t)=>{let n=document.getElementById(e).value.trim().toLowerCase(),r=document.querySelectorAll(`#${t} tbody tr`);!r||r.length===0||r.forEach(e=>{if(e.innerText.includes(`Nenhum`)||e.innerText.includes(`Nenhuma`))return;let t=e.innerText.toLowerCase();e.style.display=t.includes(n)?``:`none`})},E.toggleCheckMassa=(e,t)=>{document.querySelectorAll(`.`+t).forEach(t=>t.checked=e.checked)},E.excluirLotePedagogico=(e,t,n)=>{let r=document.querySelectorAll(`.`+t+`:checked`);if(r.length===0)return E.showToast(`Selecione pelo menos um item para excluir.`,`warning`);E.confirmar(`Excluir em Massa`,`Deseja realmente excluir <b>${r.length}</b> iten(s) selecionado(s)? Esta ação é irreversível.`,`Excluir Tudo`,`#e74c3c`,async()=>{document.body.style.cursor=`wait`;try{let t=Array.from(r).map(t=>E.api(`${e}/${t.value}`,`DELETE`));await Promise.all(t),E.showToast(`${r.length} iten(s) excluído(s) com sucesso!`,`success`);let i=document.getElementById(`app-content`);i&&(i.innerHTML=`<p style="text-align:center; color:#666; padding:20px;">Atualizando dados... ⏳</p>`),await n()}catch{E.showToast(`Erro ao excluir itens em massa.`,`error`)}finally{document.body.style.cursor=`default`}})},E.renderizarPlanejamentoPro=()=>{E.setTitulo(`Planejamento`);let e=document.getElementById(`app-content`),t=e=>`cursor:pointer; background:white; border:2px solid #eee; padding:30px; border-radius:15px; width:220px; transition:0.3s; box-shadow:0 5px 15px rgba(0,0,0,0.05);`,n=e=>`this.style.borderColor='${e}'; this.style.transform='translateY(-5px)'`,r=`this.style.borderColor='#eee'; this.style.transform='translateY(0)'`;e.innerHTML=`
        ...
        `}catch{o.innerHTML=`<p style="color:red; text-align:center;">Erro ao carregar a pauta.</p>`}},E.salvarNotasLote=async()=>{let e=document.getElementById(`nota-disc`).value,t=document.getElementById(`nota-tipo`).value;t===`Outro`&&(t=document.getElementById(`nota-outro-desc`).value||`Outro`);let n=document.getElementById(`nota-data`).value,r=document.getElementById(`nota-max`).value,i=document.getElementById(`nota-periodo`).value,a=document.querySelectorAll(`.linha-nota`);if(a.length===0)return;if(n>new Date().toISOString().split(`T`)[0])return E.showToast(`Bloqueado: Não é possível gravar notas com datas futuras.`,`error`);let o=document.querySelector(`button[onclick="App.salvarNotasLote()"]`),s=o.innerText;o.innerText=`A arquivar... ⏳`,o.disabled=!0,document.body.style.cursor=`wait`;try{let o=[],s=await E.api(`/avaliacoes?_t=${Date.now()}`);a.forEach(a=>{let c=a.getAttribute(`data-id`),l=a.getAttribute(`data-nome`),u=a.getAttribute(`data-id-avaliacao`),d=a.querySelector(`.valor-nota`).value;if(d===``)return;let f=null;f=u?s.find(e=>e.id===u):s.find(r=>r.idAluno===c&&r.data===n&&r.disciplina===e&&r.tipo===t);let p={idAluno:c,nomeAluno:l,disciplina:e,tipo:t,data:n,valorMax:r,nota:d,periodo:i,bimestre:i,dataLancamento:new Date().toISOString().split(`T`)[0]};f?o.push(E.api(`/avaliacoes/${f.id}`,`PUT`,{...f,nota:d,valorMax:r,data:n,disciplina:e,tipo:t,periodo:i,bimestre:i})):(p.id=typeof crypto.randomUUID==`function`?crypto.randomUUID():Date.now().toString(36)+Math.random().toString(36).substr(2),o.push(E.api(`/avaliacoes`,`POST`,p)))}),await Promise.all(o),E.showToast(`Pauta de notas arquivada com sucesso!`,`success`),E.cancelarEdicaoNota();let c=document.getElementById(`app-content`);c&&(c.innerHTML=`<p style="text-align:center; color:#666; padding:20px;">Atualizando avaliações... ⏳</p>`),await E.renderizarAvaliacoesPro()}catch{E.showToast(`Erro ao salvar as notas.`,`error`)}finally{o&&(o.innerText=s,o.disabled=!1),document.body.style.cursor=`default`}},E.excluirAvaliacao=e=>{E.confirmar(`Excluir Nota`,`Deseja mesmo excluir esta avaliação? Isto irá afetar o boletim do aluno.`,`Excluir`,`#e74c3c`,async()=>{await E.api(`/avaliacoes/${e}`,`DELETE`);let t=document.getElementById(`app-content`);t&&(t.innerHTML=`<p style="text-align:center; color:#666; padding:20px;">Atualizando avaliações... ⏳</p>`),await E.renderizarAvaliacoesPro()})},E.cachePedagogico={chamadas:null,alunos:null,turmas:null,planejamentos:null},E.renderizarChamadaPro=async()=>{E.setTitulo(`Controle de Presença`);let e=document.getElementById(`app-content`);e.innerHTML=`<p style="text-align:center; padding:20px; color:#666;">A carregar dados rapidamente... ⚡</p>`;try{let[t,n,r,i]=await Promise.all([E.api(`/alunos`),E.api(`/turmas`),E.api(`/chamadas?_t=${Date.now()}`),E.api(`/planejamentos?_t=${Date.now()}`)]);E.cachePedagogico.alunos=t,E.cachePedagogico.turmas=n,E.cachePedagogico.chamadas=Array.isArray(r)?r:[],E.cachePedagogico.planejamentos=i;let a=E.cachePedagogico.planejamentos.filter(e=>e.status!==`Arquivado`).map(e=>e.idAluno),o=[...E.cachePedagogico.chamadas].sort((e,t)=>new Date(t.data)-new Date(e.data)),s=E.cachePedagogico.alunos.filter(e=>(!e.status||e.status===`Ativo`)&&a.includes(e.id)),c=`<option value="">-- Turma Completa --</option>`+E.cachePedagogico.turmas.map(e=>`<option value="${E.escapeHTML(e.nome)}">${E.escapeHTML(e.nome)}</option>`).join(``),l=`<option value="">-- Aluno Específico --</option>`+s.map(e=>`<option value="${e.id}">${E.escapeHTML(e.nome)}</option>`).join(``),u=new Date().toISOString().split(`T`)[0],d=`
            ...
            <div style="display: flex; flex-wrap: wrap; gap: 20px; align-items: flex-end;">
                ${O(`Data da Aula:`,`chamada-data`,`date`,u,`max="${u}"`)}
            ...
            <div class="table-responsive-wrapper">
                <table id="tabela-historico-chamadas" style="width:100%; border-collapse:collapse; font-size:13px;">
                    ...
                    <tbody>
                        ...
                        ${o.map(e=>{let t=`#333`;return e.status===`Presença`?t=`green`:e.status===`Falta`?t=`red`:e.status===`Reposição`&&(t=`#2980b9`),`
                            <tr style="border-bottom:1px solid #eee;">
                                ...
                                <td style="padding:12px; font-weight:bold;">${E.escapeHTML(e.nomeAluno)}</td>
                                <td style="padding:12px; font-weight:bold; color:${t};">${E.escapeHTML(e.status)}</td>
                                <td style="padding:12px; color:#555;">${E.escapeHTML(e.duracao)}</td>
                                <td style="padding:12px; text-align:right;">
                                    <button onclick="App.editarLancamentoChamada('${e.id}')" style="background:none; border:none; cursor:pointer; font-size:16px; margin-right:5px;" title="Editar">✏️</button>
        ...
        `;e.innerHTML=E.UI.card(`📝 Registo de Frequência`,``,d,`100%`)+`<div id="area-lista-chamada" style="margin-top:20px;"></div><div style="margin-top:20px;">`+E.UI.card(`Histórico Completo de Lançamentos`,``,f,`100%`)+`</div>`}catch{e.innerHTML=`Erro ao carregar módulo de chamada.`}},E.editarLancamentoChamada=async e=>{let t=E.cachePedagogico.chamadas.find(t=>String(t.id)===String(e));if(!t)return E.showToast(`Erro ao localizar o registo.`,`error`);document.getElementById(`chamada-aluno`).value=t.idAluno,document.getElementById(`chamada-turma`).value=``,document.getElementById(`chamada-data`).value=t.data,document.getElementById(`chamada-duracao`).value=t.duracao,document.getElementById(`btn-cancel-chamada`).style.display=`inline-block`,E.idChamadaEditando=e,E.carregarListaChamada(),document.querySelector(`.card`).scrollIntoView({behavior:`smooth`})},E.cancelarEdicaoChamada=()=>{E.idChamadaEditando=null,document.getElementById(`chamada-aluno`).value=``,document.getElementById(`btn-cancel-chamada`).style.display=`none`,document.getElementById(`area-lista-chamada`).innerHTML=``,E.showToast(`Modo de edição cancelado.`,`info`)},E.marcarTodosChamada=e=>{let t=document.querySelectorAll(`.status-chamada`);t.length!==0&&(t.forEach(t=>{t.value=e,e===`Falta`?t.style.color=`#e74c3c`:e===`Reposição`?t.style.color=`#f39c12`:e===`Presença`?t.style.color=`#27ae60`:t.style.color=`#333`}),E.showToast(`Lote aplicado: Todos como ${e}!`,`info`))},E.carregarListaChamada=async()=>{let e=document.getElementById(`chamada-turma`).value,t=document.getElementById(`chamada-aluno`).value,n=document.getElementById(`chamada-data`).value;if(!e&&!t)return E.showToast(`Selecione uma Turma OU um Aluno específico.`,`warning`);if(!n)return E.showToast(`Preencha a Data da aula.`,`warning`);if(n>new Date().toISOString().split(`T`)[0])return E.showToast(`Não é permitido abrir grelhas de frequência para datas futuras.`,`warning`);let r=document.getElementById(`area-lista-chamada`);r.innerHTML=`<p style="text-align:center; padding:20px;">A preparar diário de classe super rápido... ⚡</p>`;try{let i=E.cachePedagogico.alunos,a=E.cachePedagogico.chamadas,o=E.cachePedagogico.planejamentos.filter(e=>e.status!==`Arquivado`).map(e=>e.idAluno),s=[];if(s=t?i.filter(e=>e.id===t):i.filter(t=>t.turma===e),s=s.filter(e=>(!e.status||e.status===`Ativo`)&&o.includes(e.id)),s.length===0){r.innerHTML=`<div class="card"><p style="text-align:center; color:#999; margin:0;">Nenhum aluno com Planejamento Ativo foi encontrado para este filtro.</p></div>`;return}let c=a.filter(e=>e.data===n),l=``;s.forEach(e=>{let t=null;t=E.idChamadaEditando&&e.id===document.getElementById(`chamada-aluno`).value?a.find(e=>String(e.id)===String(E.idChamadaEditando)):c.find(t=>t.idAluno===e.id);let n=t?t.status:`Presença`,r=E.idChamadaEditando&&t?`data-id-chamada="${t.id}"`:``;l+=`
            ...
            </tr>`});let u=E.idChamadaEditando?`<div style="background:#f39c12; color:#fff; padding:10px; text-align:center; font-weight:bold; margin-bottom:10px; border-radius:5px; animation: pop 0.3s;">⚠️ MODO DE EDIÇÃO ATIVO (A atualizar o registo existente)</div>`:``,d=E.idChamadaEditando?``:`
