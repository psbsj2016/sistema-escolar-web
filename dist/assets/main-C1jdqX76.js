...
<div style="font-size:10px; color:#999;">Emissão: ${i}</div>
        ...
        `,p.appendChild(y)}catch{A.showToast(`Erro ao gerar o documento.`,`error`)}finally{a.innerText=o,a.disabled=!1,document.body.style.cursor=`default`}},A.renderizarMenuCertificados=async()=>{let e=document.getElementById(`app-content`);e.innerHTML=`<p style="text-align:center; padding:20px; color:#666;">Preparando ambiente... 🎓</p>`;try{let t=(await A.api(`/alunos`)).filter(e=>!e.status||e.status===`Ativo`);e.innerHTML=`
