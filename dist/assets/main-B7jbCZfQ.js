...
Qualquer dúvida, estamos à inteira disposição. Tenha um excelente dia! 🌟`,s=`https://api.whatsapp.com/send?phone=${a}&text=${encodeURIComponent(o)}`;window.open(s,`_blank`)}catch(e){console.error(`Erro ao gerar link do WhatsApp:`,e),W.showToast(`Erro ao processar a mensagem.`,`error`)}},window.App=window.App||{};var G=window.App,K={Evento:{bg:`#2ecc71`,text:`#fff`},Feriado:{bg:`#e74c3c`,text:`#fff`},Prova:{bg:`#3498db`,text:`#fff`},Reunião:{bg:`#f39c12`,text:`#fff`}},q=(e,t,n=`text`,r=``,i=``)=>`
    ...
    `,o.appendChild(s),document.body.appendChild(o),setTimeout(()=>{o.style.opacity=`1`,s.style.transform=`scale(1)`},10);let c=()=>{o.style.opacity=`0`,s.style.transform=`scale(0.9)`,setTimeout(()=>document.body.removeChild(o),200)};document.getElementById(`btn-no-${a}`).onclick=c,document.getElementById(`btn-yes-${a}`).onclick=()=>{c(),i()}},G.filtrarTabela=(e,t)=>{let n=document.getElementById(e).value.trim().toLowerCase(),r=document.querySelectorAll(`#${t} tbody tr`);!r||r.length===0||r.forEach(e=>{if(e.innerText.includes(`Nenhum`)||e.innerText.includes(`Nenhuma`))return;let t=e.innerText.toLowerCase();e.style.display=t.includes(n)?``:`none`})},G.toggleCheckMassa=(e,t)=>{document.querySelectorAll(`.`+t).forEach(t=>t.checked=e.checked)},G.excluirLotePedagogico=(e,t,n)=>{let r=document.querySelectorAll(`.`+t+`:checked`);if(r.length===0)return G.showToast(`Selecione pelo menos um item para excluir.`,`warning`);G.confirmar(`Excluir em Massa`,`Deseja realmente excluir <b>${r.length}</b> iten(s) selecionado(s)? Esta ação é irreversível.`,`Excluir Tudo`,`#e74c3c`,async()=>{document.body.style.cursor=`wait`;try{let t=Array.from(r).map(t=>G.api(`${e}/${t.value}`,`DELETE`));await Promise.all(t),G.showToast(`${r.length} iten(s) excluído(s) com sucesso!`,`success`);let i=document.getElementById(`app-content`);i&&(i.innerHTML=`<p style="text-align:center; color:#666; padding:20px;">Atualizando dados... ⏳</p>`),await n()}catch{G.showToast(`Erro ao excluir itens em massa.`,`error`)}finally{document.body.style.cursor=`default`}})},G.renderizarPlanejamentoPro=()=>{G.setTitulo(`Planejamento`);let e=document.getElementById(`app-content`),t=e=>`cursor:pointer; background:white; border:2px solid #eee; padding:30px; border-radius:15px; width:220px; transition:0.3s; box-shadow:0 5px 15px rgba(0,0,0,0.05);`,n=e=>`this.style.borderColor='${e}'; this.style.transform='translateY(-5px)'`,r=`this.style.borderColor='#eee'; this.style.transform='translateY(0)'`;e.innerHTML=`
        ...
        </div>`},G.renderizarNovoPlanejamento=async()=>{let e=document.getElementById(`app-content`);e.innerHTML=`A carregar...`;try{let t=`
        ...
        `;e.innerHTML=G.UI.card(``,``,t,`800px`)}catch{e.innerHTML=`Erro.`}},G.renderizarPlanejamentosSalvos=async()=>{let e=document.getElementById(`app-content`);e.innerHTML=`A carregar...`;try{let t=(await G.api(`/planejamentos?_t=${Date.now()}`)).filter(e=>e.status!==`Arquivado`);if(t.length===0){e.innerHTML=G.UI.card(``,``,`<h3>Nenhum planejamento ativo.</h3><button onclick="App.renderizarPlanejamentoPro()" class="btn-primary">Voltar</button>`,`text-align:center; padding:40px;`);return}let n=t.map(e=>`
            <tr style="border-bottom:1px solid #eee;">
        ...
        `)}catch{e.innerHTML=`Erro.`}},G.renderizarPlanejamentosArquivados=async()=>{let e=document.getElementById(`app-content`);e.innerHTML=`A carregar arquivados...`;try{let t=(await G.api(`/planejamentos?_t=${Date.now()}`)).filter(e=>e.status===`Arquivado`);if(t.length===0){e.innerHTML=G.UI.card(``,``,`<h3>Nenhum planejamento no arquivo morto.</h3><button onclick="App.renderizarPlanejamentoPro()" class="btn-primary">Voltar</button>`,`text-align:center; padding:40px;`);return}let n=t.map(e=>`
