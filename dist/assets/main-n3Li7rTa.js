...
`;A.renderizarSelecaoRelatorio=async()=>{let e=document.getElementById(`app-content`);e.innerHTML=`<p style="text-align:center; padding:20px; color:#666;">A carregar períodos disponíveis... ⏳</p>`;try{let t=await A.api(`/financeiro`),n=new Set,r=new Date().getFullYear();n.add(r),n.add(r+1),t.forEach(e=>{if(e.vencimento){let t=parseInt(e.vencimento.split(`-`)[0]);isNaN(t)||n.add(t)}if(e.dataPagamento){let t=parseInt(e.dataPagamento.split(`-`)[0]);isNaN(t)||n.add(t)}});let i=Array.from(n).sort((e,t)=>t-e).map(e=>`<option value="${e}" ${e===r?`selected`:``}>${e}</option>`).join(``),a=[`Janeiro`,`Fevereiro`,`Março`,`Abril`,`Maio`,`Junho`,`Julho`,`Agosto`,`Setembro`,`Outubro`,`Novembro`,`Dezembro`].map((e,t)=>`<option value="${t+1}" ${t===new Date().getMonth()?`selected`:``}>${e}</option>`).join(``),o=`
        ...
        `;e.innerHTML=A.UI.card(``,``,o,`100%`)}catch(t){console.error(`Erro ao carregar anos para relatório:`,t),e.innerHTML=`<p style="color:red; text-align:center;">Erro ao ligar ao servidor para ler o histórico. Tente novamente.</p>`}},A.gerarRelatorioAnual=async()=>{let e=document.getElementById(`rel-ano`).value,t=document.getElementById(`filtro-auditoria-limpa`)?document.getElementById(`filtro-auditoria-limpa`).checked:!1,n=document.getElementById(`app-content`);n.innerHTML=`<p style="text-align:center;">A gerar relatório anual...</p>`;try{let[r,i,a]=await Promise.all([A.api(`/financeiro`),A.api(`/escola`)||{nome:`ESCOLA`,cnpj:``},A.api(`/alunos`)||[]]),o=i.foto?`<img src="${i.foto}" style="height:50px; object-fit:contain;">`:``,s={};a.forEach(e=>{s[e.id]=e.status?e.status.toLowerCase():`ativo`});let c=r.filter(t=>t.vencimento&&t.vencimento.startsWith(e));t&&(c=c.filter(e=>{if(!e.idAluno)return!0;let t=s[e.idAluno]||`ativo`,n=(e.alunoNome||``).toLowerCase();return!(t===`excluído`||t===`inativo`||n.includes(`teste`))})),c.sort((e,t)=>new Date(e.vencimento)-new Date(t.vencimento));let l=e=>parseFloat(e.valorPago1||e.valor||0)+parseFloat(e.valorPago2||0),u=c.reduce((e,t)=>e+(parseFloat(t.valor)||0),0),d=c.filter(e=>e.status===`Pago`).reduce((e,t)=>e+l(t),0),f=c.filter(e=>e.status!==`Pago`).reduce((e,t)=>e+(parseFloat(t.valor)||0),0),p=e=>e.toLocaleString(`pt-BR`,{style:`currency`,currency:`BRL`});n.innerHTML=`
            ${N}
            ...
            <div class="print-sheet">
                ...
                <div class="kpi-container" style="background:#fafafa; padding:20px; border-radius:8px; margin-bottom:30px; border:1px solid #eee;">
                    <div class="kpi-box"><div style="font-size:12px; font-weight:bold; color:#555;">TOTAL LANÇADO:</div><div style="font-size:18px; font-weight:bold; color:#333;">${p(u)}</div></div>
                    <div class="kpi-box"><div style="font-size:12px; font-weight:bold; color:green;">TOTAL RECEBIDO:</div><div style="font-size:18px; color:green;">${p(d)}</div></div>
                    <div class="kpi-box"><div style="font-size:12px; font-weight:bold; color:red;">TOTAL PENDENTE:</div><div style="font-size:18px; color:red;">${p(f)}</div></div>
                </div>
                ...
                <div class="table-responsive">
                    <table style="width:100%; border-collapse:collapse; font-size:11px; color:#555; min-width:600px;">
                        ...
                        <tbody>
                            ${c.map(e=>{let t=e.status===`Pago`,n=t?`PAGO`:`ABERTO`;return t&&e.formaPagamento&&(n+=`<br><span style="font-size:9px; color:#666; display:block; line-height:1.2; margin-top:2px;">${A.escapeHTML(e.formaPagamento)}${e.formaPagamento2?` / ${A.escapeHTML(e.formaPagamento2)}`:``}<br>Pago em: ${A.escapeHTML(e.dataPagamento?e.dataPagamento.split(`-`).reverse().join(`/`):``)}</span>`),`<tr style="border-bottom:1px solid #eee;"><td style="padding:10px; white-space:nowrap;">${A.escapeHTML(e.vencimento.split(`-`).reverse().join(`/`))}</td><td style="padding:10px;">${A.escapeHTML(e.alunoNome||`Não informado`)}</td><td style="padding:10px;">${A.escapeHTML(e.descricao)}</td><td style="padding:10px; text-align:center; font-weight:bold; color:${t?`green`:`red`};">${n}</td><td style="padding:10px; text-align:right; white-space:nowrap;">${t?p(l(e)):`-`}</td><td style="padding:10px; text-align:right; white-space:nowrap;">${t?`-`:p(parseFloat(e.valor)||0)}</td></tr>`}).join(``)}
                        </tbody>
                        <tfoot>
                            <tr style="background:#f9f9f9; font-weight:bold; border-top:2px solid #333;"><td colspan="4" style="padding:15px; text-align:right;">SALDO FINAL:</td><td style="padding:15px; text-align:right; color:green; white-space:nowrap;">${p(d)}</td><td style="padding:15px; text-align:right; color:red; white-space:nowrap;">${p(f)}</td></tr>
                        </tfoot>
                    </table>
                </div>
            </div>`}catch{A.showToast(`Erro ao gerar relatório.`,`error`)}},A.gerarRelatorioMensal=async()=>{let e=document.getElementById(`rel-ano`).value,t=parseInt(document.getElementById(`rel-mes`).value),n=document.getElementById(`filtro-auditoria-limpa`)?document.getElementById(`filtro-auditoria-limpa`).checked:!1,r=document.getElementById(`app-content`);r.innerHTML=`<p style="text-align:center;">A gerar relatório mensal...</p>`;let i=[`JANEIRO`,`FEVEREIRO`,`MARÇO`,`ABRIL`,`MAIO`,`JUNHO`,`JULHO`,`AGOSTO`,`SETEMBRO`,`OUTUBRO`,`NOVEMBRO`,`DEZEMBRO`][t-1];try{let[a,o,s]=await Promise.all([A.api(`/financeiro`),A.api(`/escola`)||{nome:`ESCOLA`,cnpj:``},A.api(`/alunos`)||[]]),c=o.foto?`<img src="${o.foto}" style="height:50px; object-fit:contain;">`:``,l={};s.forEach(e=>{l[e.id]=e.status?e.status.toLowerCase():`ativo`});let u=a.filter(n=>{if(!n.vencimento)return!1;let r=new Date(n.vencimento+`T00:00:00`);return r.getFullYear()==e&&r.getMonth()+1==t});n&&(u=u.filter(e=>{if(!e.idAluno)return!0;let t=l[e.idAluno]||`ativo`,n=(e.alunoNome||``).toLowerCase();return!(t===`excluído`||t===`inativo`||n.includes(`teste`))})),u.sort((e,t)=>new Date(e.vencimento)-new Date(t.vencimento));let d=e=>parseFloat(e.valorPago1||e.valor||0)+parseFloat(e.valorPago2||0),f=u.reduce((e,t)=>e+(parseFloat(t.valor)||0),0),p=u.filter(e=>e.status===`Pago`).reduce((e,t)=>e+d(t),0),m=u.filter(e=>e.status!==`Pago`).reduce((e,t)=>e+(parseFloat(t.valor)||0),0),h=e=>e.toLocaleString(`pt-BR`,{style:`currency`,currency:`BRL`});r.innerHTML=`
            ${N}
            <div class="no-print" style="margin-bottom:20px; text-align:center;">
                <button onclick="App.renderizarSelecaoRelatorio()" class="btn-cancel" style="padding:10px 20px; margin-right:10px; margin-bottom:10px;">⬅ VOLTAR</button>
            ...
            <div class="print-sheet">
                ...
                <div class="kpi-container" style="margin-bottom:30px;">
                    <div class="kpi-box" style="background:#34495e; color:white;"><div style="font-size:10px; text-transform:uppercase; opacity:0.8;">PREVISÃO</div><div style="font-size:20px; font-weight:bold;">${h(f)}</div></div>
                    <div class="kpi-box" style="background:#27ae60; color:white;"><div style="font-size:10px; text-transform:uppercase; opacity:0.8;">REALIZADO</div><div style="font-size:20px; font-weight:bold;">${h(p)}</div></div>
                    <div class="kpi-box" style="background:#e74c3c; color:white;"><div style="font-size:10px; text-transform:uppercase; opacity:0.8;">PENDENTE</div><div style="font-size:20px; font-weight:bold;">${h(m)}</div></div>
                </div>
                ...
                <div class="table-responsive">
                    <table style="width:100%; border-collapse:collapse; font-size:11px; color:#555; min-width:500px;">
                        ...
                        <tbody>
                            ${u.map(e=>{let t=e.status===`Pago`,n=t?`PAGO`:`PENDENTE`;return t&&e.formaPagamento&&(n+=`<br><span style="font-size:9px; color:#666; display:block; line-height:1.2; margin-top:2px;">${A.escapeHTML(e.formaPagamento)}${e.formaPagamento2?` / ${A.escapeHTML(e.formaPagamento2)}`:``}<br>Pago em: ${A.escapeHTML(e.dataPagamento?e.dataPagamento.split(`-`).reverse().join(`/`):``)}</span>`),`<tr style="border-bottom:1px solid #eee;"><td style="padding:10px; white-space:nowrap;">${A.escapeHTML(e.vencimento.split(`-`).reverse().join(`/`))}</td><td style="padding:10px;">${A.escapeHTML(e.alunoNome||`Não informado`)}</td><td style="padding:10px;">${A.escapeHTML(e.descricao)}</td><td style="padding:10px; text-align:center; font-weight:bold; color:${t?`green`:`red`};">${n}</td><td style="padding:10px; text-align:right; white-space:nowrap;">${h(t?d(e):parseFloat(e.valor)||0)}</td></tr>`}).join(``)}
                        </tbody>
                    </table>
                </div>
