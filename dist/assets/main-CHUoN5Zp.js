...
`,M=async()=>{k.verificarNotificacoes(),k.setTitulo(`Visão Geral`);let e=document.getElementById(`app-content`);try{let[t,n,r,i]=await Promise.all([k.api(`/alunos`),k.api(`/financeiro`),k.api(`/turmas`),k.api(`/cursos`)]),a=(Array.isArray(t)?t:[]).filter(e=>!e.status||e.status===`Ativo`),o=Array.isArray(n)?n:[],s=Array.isArray(r)?r:[],c=Array.isArray(i)?i:[],l=new Date,u=l.getMonth()+1,d=l.getFullYear(),f=a.map(e=>e.id),p=o.filter(e=>{if(!e.vencimento)return!1;let t=e.vencimento.split(`-`);return parseInt(t[1])===u&&parseInt(t[0])===d}),m=p.filter(e=>e.status===`Pago`).reduce((e,t)=>e+parseFloat(t.valor),0),h=p.filter(e=>e.status!==`Pago`&&f.includes(e.idAluno)).reduce((e,t)=>e+parseFloat(t.valor),0),g=o.filter(e=>e.status===`Pendente`&&new Date(e.vencimento+`T00:00:00`)<l&&f.includes(e.idAluno)).sort((e,t)=>new Date(e.vencimento)-new Date(t.vencimento)),_=e=>e.toLocaleString(`pt-BR`,{minimumFractionDigits:2,maximumFractionDigits:2}),v=k.usuario?k.usuario.tipo:`Gestor`,y=v!==`Professor`,b=g.length===0?`<div style="text-align:center; padding:20px; color:#27ae60; font-weight:bold; font-size:14px;">🎉 Excelente! Nenhum título em atraso.</div>`:g.map(e=>{let t=(a.find(t=>t.id===e.idAluno)||{}).whatsapp||``,n=e.vencimento.split(`-`).reverse().join(`/`),r=_(parseFloat(e.valor));return j(e.alunoNome||`Desconhecido`,n,r,t)}).join(``),x=JSON.parse(localStorage.getItem(k.getTenantKey(`escola_atalhos`)));(!x||!Array.isArray(x)||x.length===0)&&(x=[`novo_aluno`,`fin_carne`,`ped_chamada`,`ped_notas`,`ped_plan`,`ped_bol`]);let S=x.map(e=>{let t=(window.LISTA_FUNCIONALIDADES||[]).find(t=>t.id===e);return t&&t.roles.includes(v)?`<div class="shortcut-btn" onclick="${t.acao}"><div>${t.icon}</div><span>${t.nome}</span></div>`:``}).join(``);if(e.innerHTML=`
        ...
        `},mostrarAreaLinks:async()=>{let e=document.getElementById(`area-dinamica-hub`);e.innerHTML=`<p style="text-align:center; padding:20px; color:#666;">A carregar links... ⏳</p>`;try{let t=await F.api(`/escola`);if(!t||t.error||!t.escolaId){e.innerHTML=`
