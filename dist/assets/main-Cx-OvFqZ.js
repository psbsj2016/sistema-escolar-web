...
`;A.renderizarSelecaoRelatorio=async()=>{let e=document.getElementById(`app-content`);e.innerHTML=`<p style="text-align:center; padding:20px; color:#666;">A carregar períodos disponíveis... ⏳</p>`;try{let t=await A.api(`/financeiro`),n=new Set,r=new Date().getFullYear();n.add(r),n.add(r+1),t.forEach(e=>{if(e.vencimento){let t=parseInt(e.vencimento.split(`-`)[0]);isNaN(t)||n.add(t)}if(e.dataPagamento){let t=parseInt(e.dataPagamento.split(`-`)[0]);isNaN(t)||n.add(t)}});let i=Array.from(n).sort((e,t)=>t-e).map(e=>`<option value="${e}" ${e===r?`selected`:``}>${e}</option>`).join(``),a=[`Janeiro`,`Fevereiro`,`Março`,`Abril`,`Maio`,`Junho`,`Julho`,`Agosto`,`Setembro`,`Outubro`,`Novembro`,`Dezembro`].map((e,t)=>`<option value="${t+1}" ${t===new Date().getMonth()?`selected`:``}>${e}</option>`).join(``),o=`
    ...
    `;e.innerHTML=A.UI.card(``,``,i,`100%`)},A.renderizarMenuDocumentosOficiais=async()=>{let e=document.getElementById(`app-content`);e.innerHTML=`<p style="text-align:center; padding:20px; color:#666;">A carregar base de alunos... ⏳</p>`;try{let t=(await A.api(`/alunos`)).filter(e=>!e.status||e.status===`Ativo`);e.innerHTML=`
        ...
        `}catch{e.innerHTML=`<p>Erro ao carregar dados.</p>`}},A.gerarDocumentoOficialPrint=async()=>{let e=document.getElementById(`doc-aluno-oficial`).value,t=document.getElementById(`doc-tipo-oficial`).value,n=new Date().toLocaleDateString(`pt-BR`);if(!e)return A.showToast(`Selecione um aluno na lista.`,`warning`);let r=document.querySelector(`button[onclick="App.gerarDocumentoOficialPrint()"]`),i=r.innerText;r.innerText=`A Processar... ⏳`,r.disabled=!0,document.body.style.cursor=`wait`;try{let r=(await A.api(`/alunos`)).find(t=>t.id===e)||{},i=await A.api(`/escola`)||{nome:`A INSTITUIÇÃO`,cnpj:`00.000.000/0000-00`},a=i.endereco?`${i.endereco}, ${i.numero||`S/N`} - ${i.bairro||``}. ${i.cidade||``}-${i.estado||``} | CEP: ${i.cep||``}`:``,o=document.getElementById(`doc-area-oficial`);o.innerHTML=`<p style="text-align:center;">Gerando Layout... ⏳</p>`;let s=`
            <div class="doc-header" style="display:flex; justify-content:space-between; align-items:center; border-bottom:2px solid #333; padding-bottom:15px; margin-bottom:30px; flex-wrap:wrap; gap:15px;">
                <div style="display:flex; align-items:center; gap:20px;">${i.foto?`<img src="${i.foto}" style="height:60px; object-fit:contain;">`:``}<div><h2 style="margin:0; text-transform:uppercase; font-size:18px;">${A.escapeHTML(i.nome)}</h2><div style="font-size:12px; color:#555;">CNPJ: ${A.escapeHTML(i.cnpj)}<br>${A.escapeHTML(a)}</div></div></div>
                <div style="text-align:right;"><div><b>${t===`contrato`?`CONTRATO DE SERVIÇOS`:`DECLARAÇÃO DE MATRÍCULA`}</b></div><div style="font-size:10px; color:#999;">Emissão: ${n}</div></div>
            </div>`,c=`
            <div class="no-print" style="text-align:center; margin-bottom:20px;">
        ...
        `;if(t===`contrato`){let e=parseFloat(r.valorMensalidade||0).toLocaleString(`pt-BR`,{minimumFractionDigits:2});o.innerHTML=`
                ${N}
                ${c}
                <div class="print-sheet" style="font-family: Arial, sans-serif; color: #000; line-height: 1.6;">
                    ...
                    <p style="text-align: justify; margin-top: 20px; font-size:14px;">
                        ...
                        <b>${A.escapeHTML(r.nome)}</b>, portador(a) do CPF nº <b>${A.escapeHTML(r.cpf||`___________`)}</b> e RG nº <b>${A.escapeHTML(r.rg||`___________`)}</b>, 
                        residente e domiciliado(a) na ${A.escapeHTML(r.rua||``)}, ${A.escapeHTML(r.numero||``)} - ${A.escapeHTML(r.bairro||``)}, 
                        ${A.escapeHTML(r.cidade||``)}/${A.escapeHTML(r.estado||``)}, doravante denominado(a) <b>CONTRATANTE</b>.
                    </p>

                    <h4 style="margin-top:25px; margin-bottom: 5px;">CLÁUSULA PRIMEIRA - DO OBJETO</h4>
                    <p style="text-align: justify; margin-top:0; font-size:14px;">O presente contrato tem como objeto a prestação de serviços educacionais por parte da CONTRATADA ao CONTRATANTE, referente ao curso de <b>${A.escapeHTML(r.curso||`Não especificado`)}</b>, a ser ministrado na turma <b>${A.escapeHTML(r.turma||`Não especificada`)}</b>.</p>

                    <h4 style="margin-top:25px; margin-bottom: 5px;">CLÁUSULA SEGUNDA - DOS VALORES E FORMA DE PAGAMENTO</h4>
                    <p style="text-align: justify; margin-top:0; font-size:14px;">Pelos serviços educacionais prestados, o CONTRATANTE pagará à CONTRATADA a mensalidade no valor estipulado de <b>R$ ${e}</b>, com vencimento programado para todo dia <b>${A.escapeHTML(r.diaVencimento||`10`)}</b> de cada mês. O atraso no pagamento sujeitará o CONTRATANTE a multas e juros moratórios conforme a legislação vigente.</p>
                    ...
                    <p style="text-align: right; margin-top: 50px; font-size:14px;">Local e Data: ____________________________, ${n}</p>
                    ...
                    <div style="display: flex; justify-content: space-between; margin-top: 60px; text-align: center; flex-wrap:wrap; gap:30px;">
                        ...
                        <div style="flex:1; min-width:200px; border-top: 1px solid #000; padding-top: 10px; font-size:12px;">
                            <b>${A.escapeHTML(r.nome)}</b><br>CONTRATANTE
                        </div>
                    </div>
                </div>
            `;let t=document.createElement(`style`);t.innerHTML=`@media print { @page { size: A4 portrait; margin: 15mm; } }`,o.appendChild(t)}else if(t===`declaracao`){o.innerHTML=`
                ${N}
                ${c}
                <div class="print-sheet" style="font-family: Arial, sans-serif; color: #000; min-height: 297mm; display:flex; flex-direction:column;">
                    ...
                    <p style="text-align: justify; font-size: 16px; line-height: 2; margin-bottom: 30px;">
                        Declaramos para os devidos fins que <b>${A.escapeHTML(r.nome)}</b>, inscrito(a) no CPF sob o nº <b>${A.escapeHTML(r.cpf||`___________`)}</b>, 
                        encontra-se regularmente matriculado(a) e frequentando o curso de <b>${A.escapeHTML(r.curso||`Não especificado`)}</b> 
                        (Turma: ${A.escapeHTML(r.turma||`Não especificada`)}) nesta instituição de ensino.
                    </p>
                    ...
                    <p style="text-align: right; font-size: 14px; margin-bottom: 80px;">
                        Local e Data: ____________________________, ${n}.
                    </p>
            ...
            `;let e=document.createElement(`style`);e.innerHTML=`@media print { @page { size: A4 portrait; margin: 15mm; } }`,o.appendChild(e)}}catch{A.showToast(`Erro ao gerar o documento.`,`error`)}finally{r.innerText=i,r.disabled=!1,document.body.style.cursor=`default`}},A.renderizarMenuCertificados=async()=>{let e=document.getElementById(`app-content`);e.innerHTML=`<p style="text-align:center; padding:20px; color:#666;">A preparar o ambiente de graduação... 🎓</p>`;try{let t=(await A.api(`/alunos`)).filter(e=>!e.status||e.status===`Ativo`);e.innerHTML=`
        ...
        `}catch{e.innerHTML=`<p>Erro ao carregar dados.</p>`}},A.gerarCertificadoPrint=async()=>{let e=document.getElementById(`cert-aluno`).value,t=document.getElementById(`cert-carga`).value||`40`,n=document.getElementById(`cert-data-inicio`),r=n&&n.value?n.value.split(`-`).reverse().join(`/`):`-`,i=document.getElementById(`cert-data-fim`),a=new Date().toLocaleDateString(`pt-BR`),o=i&&i.value?i.value.split(`-`).reverse().join(`/`):a;if(!e)return A.showToast(`Selecione um aluno na lista.`,`warning`);let s=document.querySelector(`button[onclick="App.gerarCertificadoPrint()"]`),c=s.innerText;s.innerText=`A Criar Diploma... ⏳`,s.disabled=!0,document.body.style.cursor=`wait`;try{let n=(await A.api(`/alunos`)).find(t=>t.id===e)||{},i=await A.api(`/escola`)||{nome:`A INSTITUIÇÃO`,cnpj:`00.000.000/0000-00`},s=document.getElementById(`cert-area`);s.innerHTML=`<p style="text-align:center;">Desenhando Certificado... ⏳</p>`;let c=i.foto?`<img src="${i.foto}" style="max-height:100px; max-width:150px; object-fit:contain; filter: drop-shadow(0px 2px 4px rgba(0,0,0,0.1));">`:`<div style="font-size:16px; font-weight:bold; color:#2c3e50; border:2px solid #2c3e50; padding:15px; border-radius:50%; display:flex; align-items:center; justify-content:center; width:80px; height:80px;">SELO</div>`;s.innerHTML=`
            ${N}
                ...
                <div class="print-sheet">
                    <div class="cert-box">
                        ...
                        <p style="font-size: 19px; color: #333; max-width: 900px; margin: 0 auto; line-height: 1.8; text-align: justify; text-align-last: center;">
                            portador(a) do CPF nº <b>${A.escapeHTML(n.cpf||`Não informado`)}</b>, concluiu com êxito todos os requisitos acadêmicos do curso de <b>${A.escapeHTML(n.curso||`Não especificado`)}</b>, iniciado em <b>${r}</b> e finalizado em <b>${o}</b>, com carga horária total de <b>${t} horas</b> e aproveitamento plenamente satisfatório.
                        </p>
