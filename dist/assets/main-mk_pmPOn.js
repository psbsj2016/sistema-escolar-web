...
`;A.renderizarSelecaoRelatorio=async()=>{let e=document.getElementById(`app-content`);e.innerHTML=`<p style="text-align:center; padding:20px; color:#666;">A carregar períodos disponíveis... ⏳</p>`;try{let t=await A.api(`/financeiro`),n=new Set,r=new Date().getFullYear();n.add(r),n.add(r+1),t.forEach(e=>{if(e.vencimento){let t=parseInt(e.vencimento.split(`-`)[0]);isNaN(t)||n.add(t)}if(e.dataPagamento){let t=parseInt(e.dataPagamento.split(`-`)[0]);isNaN(t)||n.add(t)}});let i=Array.from(n).sort((e,t)=>t-e).map(e=>`<option value="${e}" ${e===r?`selected`:``}>${e}</option>`).join(``),a=[`Janeiro`,`Fevereiro`,`Março`,`Abril`,`Maio`,`Junho`,`Julho`,`Agosto`,`Setembro`,`Outubro`,`Novembro`,`Dezembro`].map((e,t)=>`<option value="${t+1}" ${t===new Date().getMonth()?`selected`:``}>${e}</option>`).join(``),o=`
            ...
            <div style="display:flex; gap:15px; flex-wrap:wrap; margin-bottom:25px;">
                ...
                ${M(`🛡️ Filtro de Auditoria:`,`filtro-auditoria`,`
        ...
        `;e.innerHTML=A.UI.card(``,``,o,`100%`)}catch(t){console.error(`Erro ao carregar anos para relatório:`,t),e.innerHTML=`<p style="color:red; text-align:center;">Erro ao ligar ao servidor para ler o histórico. Tente novamente.</p>`}},A.gerarRelatorioAnual=async()=>{let e=document.getElementById(`rel-ano`).value,t=document.getElementById(`filtro-auditoria`)?document.getElementById(`filtro-auditoria`).value:`padrao`,n=document.getElementById(`app-content`);n.innerHTML=`<p style="text-align:center;">A gerar relatório anual...</p>`;try{let[r,i,a]=await Promise.all([A.api(`/financeiro`),A.api(`/escola`)||{nome:`ESCOLA`,cnpj:``},A.api(`/alunos`)||[]]),o=i.foto?`<img src="${i.foto}" style="height:50px; object-fit:contain;">`:``,s={},c={};a.forEach(e=>{let t=e.status?e.status.toLowerCase():`ativo`;s[e.id]=t,e.nome&&(c[e.nome.toLowerCase().trim()]=t)});let l=r.filter(t=>t.vencimento&&t.vencimento.startsWith(e));l=l.filter(e=>{let n=`ativo`;e.idAluno&&s[e.idAluno]?n=s[e.idAluno]:e.alunoNome&&c[e.alunoNome.toLowerCase().trim()]&&(n=c[e.alunoNome.toLowerCase().trim()]);let r=(e.alunoNome||``).toLowerCase(),i=n===`excluído`||n===`inativo`||r.includes(`teste`),a=e.status===`Pago`;return t===`pagos`?a&&!i:t===`pendentes`?!a&&!i:t===`excluidos`?i:t===`tudo`?!0:!i}),l.sort((e,t)=>new Date(e.vencimento)-new Date(t.vencimento));let u=e=>parseFloat(e.valorPago1||e.valor||0)+parseFloat(e.valorPago2||0),d=l.reduce((e,t)=>e+(parseFloat(t.valor)||0),0),f=l.filter(e=>e.status===`Pago`).reduce((e,t)=>e+u(t),0),p=l.filter(e=>e.status!==`Pago`).reduce((e,t)=>e+(parseFloat(t.valor)||0),0),m=e=>e.toLocaleString(`pt-BR`,{style:`currency`,currency:`BRL`});n.innerHTML=`
            ${N}
            ...
            </div>`}catch{A.showToast(`Erro ao gerar relatório.`,`error`)}},A.gerarRelatorioMensal=async()=>{let e=document.getElementById(`rel-ano`).value,t=parseInt(document.getElementById(`rel-mes`).value),n=document.getElementById(`filtro-auditoria`)?document.getElementById(`filtro-auditoria`).value:`padrao`,r=document.getElementById(`app-content`);r.innerHTML=`<p style="text-align:center;">A gerar relatório mensal...</p>`;let i=[`JANEIRO`,`FEVEREIRO`,`MARÇO`,`ABRIL`,`MAIO`,`JUNHO`,`JULHO`,`AGOSTO`,`SETEMBRO`,`OUTUBRO`,`NOVEMBRO`,`DEZEMBRO`][t-1];try{let[a,o,s]=await Promise.all([A.api(`/financeiro`),A.api(`/escola`)||{nome:`ESCOLA`,cnpj:``},A.api(`/alunos`)||[]]),c=o.foto?`<img src="${o.foto}" style="height:50px; object-fit:contain;">`:``,l={},u={};s.forEach(e=>{let t=e.status?e.status.toLowerCase():`ativo`;l[e.id]=t,e.nome&&(u[e.nome.toLowerCase().trim()]=t)});let d=a.filter(n=>{if(!n.vencimento)return!1;let r=new Date(n.vencimento+`T00:00:00`);return r.getFullYear()==e&&r.getMonth()+1==t});d=d.filter(e=>{let t=`ativo`;e.idAluno&&l[e.idAluno]?t=l[e.idAluno]:e.alunoNome&&u[e.alunoNome.toLowerCase().trim()]&&(t=u[e.alunoNome.toLowerCase().trim()]);let r=(e.alunoNome||``).toLowerCase(),i=t===`excluído`||t===`inativo`||r.includes(`teste`),a=e.status===`Pago`;return n===`pagos`?a&&!i:n===`pendentes`?!a&&!i:n===`excluidos`?i:n===`tudo`?!0:!i}),d.sort((e,t)=>new Date(e.vencimento)-new Date(t.vencimento));let f=e=>parseFloat(e.valorPago1||e.valor||0)+parseFloat(e.valorPago2||0),p=d.reduce((e,t)=>e+(parseFloat(t.valor)||0),0),m=d.filter(e=>e.status===`Pago`).reduce((e,t)=>e+f(t),0),h=d.filter(e=>e.status!==`Pago`).reduce((e,t)=>e+(parseFloat(t.valor)||0),0),g=e=>e.toLocaleString(`pt-BR`,{style:`currency`,currency:`BRL`});r.innerHTML=`
            ${N}
            <div class="no-print" style="margin-bottom:20px; text-align:center;">
                <button onclick="App.renderizarSelecaoRelatorio()" class="btn-cancel" style="padding:10px 20px; margin-right:10px; margin-bottom:10px;">⬅ VOLTAR</button>
