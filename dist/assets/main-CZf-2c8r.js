...
`;A.renderizarSelecaoRelatorio=async()=>{let e=document.getElementById(`app-content`);e.innerHTML=`<p style="text-align:center; padding:20px; color:#666;">A carregar períodos disponíveis... ⏳</p>`;try{let t=await A.api(`/financeiro`),n=new Set,r=new Date().getFullYear();n.add(r),n.add(r+1),t.forEach(e=>{if(e.vencimento){let t=parseInt(e.vencimento.split(`-`)[0]);isNaN(t)||n.add(t)}if(e.dataPagamento){let t=parseInt(e.dataPagamento.split(`-`)[0]);isNaN(t)||n.add(t)}});let i=Array.from(n).sort((e,t)=>t-e).map(e=>`<option value="${e}" ${e===r?`selected`:``}>${e}</option>`).join(``),a=[`Janeiro`,`Fevereiro`,`Março`,`Abril`,`Maio`,`Junho`,`Julho`,`Agosto`,`Setembro`,`Outubro`,`Novembro`,`Dezembro`].map((e,t)=>`<option value="${t+1}" ${t===new Date().getMonth()?`selected`:``}>${e}</option>`).join(``),o=`
    ...
    `;e.innerHTML=A.UI.card(``,``,r,`500px`)},A.gerarDossie=async()=>{let e=document.getElementById(`dossie-ano`).value,t=parseInt(document.getElementById(`dossie-mes`).value),n=[`Janeiro`,`Fevereiro`,`Março`,`Abril`,`Maio`,`Junho`,`Julho`,`Agosto`,`Setembro`,`Outubro`,`Novembro`,`Dezembro`],r=n[t-1],i=t-1,a=parseInt(e);i===0&&(i=12,--a);let o=n[i-1],s=document.getElementById(`app-content`);s.innerHTML=`<p style="text-align:center; padding:15px; font-size:13px;">Processando Dashboard Corporativo... ⏳</p>`,document.body.style.cursor=`wait`;try{let[n,c,l,u,d]=await Promise.all([A.api(`/alunos`),A.api(`/turmas`),A.api(`/cursos`),A.api(`/financeiro`),A.api(`/escola`)]),f=new Date,p=e=>parseFloat(e||0).toLocaleString(`pt-BR`,{style:`currency`,currency:`BRL`}),m=e=>parseFloat(e.valorPago1||e.valor)+parseFloat(e.valorPago2||0),h=e=>e.descricao&&e.descricao.toLowerCase().includes(`venda`)||e.idCarne&&e.idCarne.includes(`VENDA`),g=u.filter(t=>t.vencimento&&t.vencimento.startsWith(e)&&t.tipo===`Receita`),_=g.filter(e=>parseInt(e.vencimento.split(`-`)[1])===t),v=g.filter(e=>e.status===`Pago`).reduce((e,t)=>e+m(t),0),y=g.filter(e=>!h(e)).reduce((e,t)=>e+(parseFloat(t.valor)||0),0),b=_.filter(e=>!h(e)).reduce((e,t)=>e+(parseFloat(t.valor)||0),0),x=u.filter(e=>e.status===`Pendente`&&e.tipo===`Receita`&&new Date(e.vencimento+`T00:00:00`)<f).reduce((e,t)=>e+(parseFloat(t.valor)||0),0),S=_.filter(e=>e.status===`Pago`&&!h(e)).reduce((e,t)=>e+m(t),0),C=_.filter(e=>e.status===`Pago`&&h(e)).reduce((e,t)=>e+m(t),0),w=S+C,T={},E={};_.filter(e=>e.status===`Pago`).forEach(e=>{let t=h(e)?E:T,n=e.formaPagamento||`Outros`,r=e.formaPagamento2;t[n]=(t[n]||0)+parseFloat(e.valorPago1||e.valor),r&&(t[r]=(t[r]||0)+parseFloat(e.valorPago2||0))});let D=``,O=0;for(let e=1;e<=t;e++){let t=g.filter(t=>parseInt(t.vencimento.split(`-`)[1])===e&&t.status===`Pago`),n=t.filter(e=>!h(e)).reduce((e,t)=>e+m(t),0),r=t.filter(e=>h(e)).reduce((e,t)=>e+m(t),0),i=n+r;O+=i,D+=`<tr style="border-bottom:1px solid #f2f2f2;">
                <td style="padding:1px 4px; font-weight:bold;">${[`Jan`,`Fev`,`Mar`,`Abr`,`Mai`,`Jun`,`Jul`,`Ago`,`Set`,`Out`,`Nov`,`Dez`][e-1]}</td>
                <td style="padding:1px 4px; text-align:right; color:#2980b9;">${p(n)}</td>
                <td style="padding:1px 4px; text-align:right; color:#8e44ad;">${p(r)}</td>
                <td style="padding:1px 4px; text-align:right; font-weight:bold; color:#333;">${p(i)}</td>
            </tr>`}let k={};c.forEach(e=>k[e.nome]=0);let j={};l.forEach(e=>j[e.nome]=0);let M={ativo:{total:0,masc:0,fem:0,color:`#27ae60`,label:`Ativos`},trancado:{total:0,masc:0,fem:0,color:`#f39c12`,label:`Trancados`},cancelado:{total:0,masc:0,fem:0,color:`#d35400`,label:`Cancelados`},excluido:{total:0,masc:0,fem:0,color:`#c0392b`,label:`Excluídos`}},P={};n.forEach(e=>{e.turma&&k[e.turma]!==void 0&&k[e.turma]++,e.curso&&j[e.curso]!==void 0&&j[e.curso]++;let t=(e.status||`ativo`).toLowerCase().trim();t=t.includes(`exclu`)?`excluido`:t.includes(`cancel`)?`cancelado`:t.includes(`tranc`)?`trancado`:`ativo`,M[t]&&(M[t].total++,e.sexo===`Masculino`?M[t].masc++:e.sexo===`Feminino`&&M[t].fem++);let n=(e.pais||e.nacionalidade||`Brasil`).trim().toUpperCase();n===``&&(n=`BRASIL`),P[n]=(P[n]||0)+1});let F=`<div style="display:grid; grid-template-columns: repeat(4, 1fr); gap:6px; margin-bottom:6px;">`;Object.keys(M).forEach(e=>{let t=M[e],n=t.total>0?Math.round(t.masc/t.total*100):0,r=t.total>0?Math.round(t.fem/t.total*100):0;F+=`
                <div style="border-left: 2px solid ${t.color}; background:#f9f9f9; padding:3px 5px; border-radius:0 3px 3px 0;">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1px;">
                        <strong style="font-size:8.5px; text-transform:uppercase; color:#555;">${t.label}</strong>
                        <span style="font-size:11px; font-weight:bold; color:${t.color};">${t.total}</span>
                    </div>
                    <div style="display:flex; justify-content:space-between; font-size:7.5px; color:#666; line-height:1;">
                        <span>👨${t.masc}(${n}%)</span>
                        <span>👩${t.fem}(${r}%)</span>
                    </div>
                </div>
            `}),F+=`</div>`;let I=`<div style="font-size:8px; color:#999; padding:5px;">Nenhum registo ativo.</div>`,L=l.length?`<div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(95px, 1fr)); gap:3px;">`+l.map(e=>`<div style="background:#f5f7f8; border:1px solid #eef0f1; padding:2px 4px; border-radius:2px; display:flex; justify-content:space-between; align-items:center; font-size:8px; line-height:1;">
                <span style="color:#333; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:75%;" title="${A.escapeHTML(e.nome)}">${A.escapeHTML(e.nome)}</span>
                <strong style="color:#2980b9;">${j[e.nome]||0}</strong>
            </div>`).join(``)+`</div>`:I,R=c.length?`<div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(95px, 1fr)); gap:3px;">`+c.map(e=>`<div style="background:#f5f7f8; border:1px solid #eef0f1; padding:2px 4px; border-radius:2px; display:flex; justify-content:space-between; align-items:center; font-size:8px; line-height:1;">
                <span style="color:#333; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:75%;" title="${A.escapeHTML(e.nome)}">${A.escapeHTML(e.nome)}</span>
                <strong style="color:#27ae60;">${k[e.nome]||0}</strong>
            </div>`).join(``)+`</div>`:I,z=u.filter(n=>n.status===`Pendente`&&n.tipo===`Receita`&&n.vencimento&&new Date(n.vencimento+`T00:00:00`).getFullYear()===parseInt(e)&&new Date(n.vencimento+`T00:00:00`).getMonth()+1===t).reduce((e,t)=>e+(parseFloat(t.valor)||0),0),B=u.filter(e=>e.status===`Pendente`&&e.tipo===`Receita`&&e.vencimento&&new Date(e.vencimento+`T00:00:00`).getFullYear()===a&&new Date(e.vencimento+`T00:00:00`).getMonth()+1===i).reduce((e,t)=>e+(parseFloat(t.valor)||0),0);s.innerHTML=`
            ${N}
            <style>
                * { box-sizing: border-box !important; }
                .exec-grid-3 { display: grid; grid-template-columns: 1fr 1fr 1.1fr; gap: 6px; margin-bottom: 6px; width: 100%; }
                .exec-grid-2 { display: grid; grid-template-columns: 1fr 1.3fr; gap: 6px; margin-bottom: 4px; width: 100%; }
                .exec-box { background: #fff; padding: 8px; border-radius: 4px; border: 1px solid #e2e8f0; box-shadow: 0 1px 2px rgba(0,0,0,0.01); page-break-inside: avoid; overflow: hidden; width: 100%; }
                .exec-title { font-size: 9.5px; text-transform: uppercase; color: #64748b; border-bottom: 1px solid #f1f5f9; padding-bottom: 3px; margin-top: 0; margin-bottom: 6px; font-weight: bold; letter-spacing: 0.3px; }
                .kpi-row { display: grid; grid-template-columns: repeat(5, 1fr); gap: 6px; margin-bottom: 6px; width: 100%; }
                .kpi-item { background: #fff; padding: 5px; border-radius: 3px; border: 1px solid #e2e8f0; text-align: center; }
                .kpi-item-title { font-size: 7.5px; text-transform: uppercase; color: #64748b; margin-bottom: 1px; font-weight: bold; }
                .kpi-item-val { font-size: 13px; font-weight: 900; }
                .exec-table { width: 100%; border-collapse: collapse; font-size: 8.5px; line-height: 1.1; }
                .exec-table th { background: #f8fafc; padding: 3px 4px; text-align: left; border-bottom: 1px solid #cbd5e1; color: #475569; font-weight: bold; }
                .exec-table td { padding: 2px 4px; border-bottom: 1px solid #f1f5f9; color: #334155; }
                
                @media print {
                    @page { margin: 8mm 6mm; size: A4 portrait; }
                    .exec-box { box-shadow: none; border: 1px solid #cbd5e1; background: #fff !important; }
                    .no-print { display: none !important; }
                    .print-sheet { padding: 0 !important; margin: 0 !important; max-width: 100% !important; width: 100% !important; border: none !important; background: transparent !important; }
            ...
            <div class="no-print" style="text-align:center; margin-bottom:15px;">
                <button onclick="App.renderizarDossie()" class="btn-cancel" style="margin-right:10px; margin-bottom:10px; padding:6px 12px; font-size:12px;">⬅ VOLTAR</button>
                <button onclick="window.print()" class="btn-primary" style="width:auto; padding:6px 15px; font-size:12px; margin-bottom:10px;">🖨️ IMPRIMIR DOSSIÊ AJUSTADO</button>
            </div>
            
            <div class="print-sheet">
                
                <div style="display:flex; justify-content:space-between; align-items:flex-end; border-bottom: 2px solid #1e293b; padding-bottom: 4px; margin-bottom:6px; width:100%;">
                    <div style="display:flex; align-items:center; gap:8px;">
                        ${d.foto?`<img src="${d.foto}" style="height:32px; object-fit:contain;">`:``} 
                        <div>
                            <h2 style="margin:0; text-transform:uppercase; color:#0f172a; font-size:13px; line-height:1; font-weight:800;">${A.escapeHTML(d.nome)}</h2>
                            <div style="font-size:8px; color:#64748b; margin-top:1px;">CNPJ: ${A.escapeHTML(d.cnpj)}</div>
                        </div>
                    </div>
                    <div style="text-align:right;">
                        <div style="font-weight:900; font-size:11px; color:#0f172a; text-transform:uppercase; line-height:1;">Dossiê Corporativo Executivo - ${r}/${e}</div>
                        <div style="font-size:7.5px; color:#64748b; margin-top:1px;">Gerado em: ${f.toLocaleDateString(`pt-BR`)} às ${f.toLocaleTimeString(`pt-BR`)}</div>
                    </div>
                </div>
                
                <div class="kpi-row">
                    <div class="kpi-item" style="border-top: 2px solid #475569;">
                        <div class="kpi-item-title">Prev. Ano (Mensal.)</div>
                        <div class="kpi-item-val" style="color:#1e293b;">${p(y)}</div>
                    </div>
                    <div class="kpi-item" style="border-top: 2px solid #d97706;">
                        <div class="kpi-item-title">Prev. Mês (Mensal.)</div>
                        <div class="kpi-item-val" style="color:#b45309;">${p(b)}</div>
                    </div>
                    <div class="kpi-item" style="border-top: 2px solid #16a34a;">
                        <div class="kpi-item-title">Arrecadado Ano</div>
                        <div class="kpi-item-val" style="color:#15803d;">${p(v)}</div>
                    </div>
                    <div class="kpi-item" style="border-top: 2px solid #2563eb;">
                        <div class="kpi-item-title">Arrecadado Mês</div>
                        <div class="kpi-item-val" style="color:#1d4ed8;">${p(w)}</div>
                    </div>
                    <div class="kpi-item" style="border-top: 2px solid #dc2626;">
                        <div class="kpi-item-title">Inad. Histórica</div>
                        <div class="kpi-item-val" style="color:#b91c1c;">${p(x)}</div>
                    </div>
                </div>
                ...
                <div class="exec-grid-3">
                    ...
                    <div class="exec-box" style="display:flex; flex-direction:column; justify-content:space-between;">
                        <div>
                            <h3 class="exec-title">Desempenho Financeiro</h3>
                            <table class="exec-table" style="margin-bottom:6px;">
                                <tr><td>Mensalidades Correntes:</td><td style="text-align:right; font-weight:bold; color:#2563eb;">${p(S)}</td></tr>
                                <tr><td>Vendas Balcão/Loja:</td><td style="text-align:right; font-weight:bold; color:#7c3aed;">${p(C)}</td></tr>
                                <tr style="background:#f8fafc; border-top:1px solid #e2e8f0;"><td style="font-weight:bold;">Faturamento Mês:</td><td style="text-align:right; font-size:10.5px; font-weight:900; color:#16a34a;">${p(w)}</td></tr>
                            </table>
                            
                            <div style="display:flex; justify-content:space-around; align-items:center; margin-bottom:4px; margin-top:2px;">
                                <div style="text-align:center;">
                                    <div style="font-size:6.5px; color:#64748b; font-weight:bold; margin-bottom:1px;">CANAIS (MENSAL.)</div>
                                    <div style="position:relative; width:55px; height:55px; margin:0 auto;"><canvas id="grafMensalidade"></canvas></div>
                                </div>
                                <div style="text-align:center;">
                                    <div style="font-size:6.5px; color:#64748b; font-weight:bold; margin-bottom:1px;">CANAIS (VENDAS)</div>
                                    <div style="position:relative; width:55px; height:55px; margin:0 auto;"><canvas id="grafVenda"></canvas></div>
                                </div>
                            </div>
                        </div>

                        <div style="display:flex; gap:4px; align-items:stretch; border-top:1px dashed #e2e8f0; padding-top:5px; margin-top:2px;">
                            <div style="flex:1; background:#fff; border: 1px solid #fee2e2; border-radius:3px; padding:4px; text-align:center;">
                                <div style="font-size:7.5px; color:#ef4444; font-weight:bold; text-transform:uppercase; line-height:1;">Inad. Ant. (${o})</div>
                                <div style="font-size:11px; font-weight:bold; color:#dc2626; margin-top:1px;">${p(B)}</div>
                            </div>
                            <div style="flex:1; background:#fef2f2; border: 1px solid #ef4444; border-radius:3px; padding:4px; text-align:center;">
                                <div style="font-size:7.5px; color:#991b1b; font-weight:bold; text-transform:uppercase; line-height:1;">Inad. Vigente (${r})</div>
                                <div style="font-size:12.5px; font-weight:900; color:#991b1b; margin-top:1px;">${p(z)}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="exec-box">
                        ...
                        <table class="exec-table">
                            ...
                            <tbody>${D}</tbody>
                            <tfoot><tr style="background:#f8fafc; border-top:1px solid #cbd5e1;"><td colspan="3" style="text-align:right; font-weight:bold; padding-top:4px;">LÍQUIDO ACUMULADO:</td><td style="text-align:right; font-weight:900; color:#16a34a; padding-top:4px;">${p(O)}</td></tr></tfoot>
                        </table>
                    </div>
                    ...
                    <div class="exec-box" style="display:flex; flex-direction:column; justify-content:space-between;">
                        ...
                        <div style="border-top:1px solid #f1f5f9; padding-top:4px; margin-top:4px;">
                            <div style="font-size:7.5px; font-weight:bold; color:#475569; text-align:center; margin-bottom:2px; text-transform:uppercase;">Distribuição por País de Origem</div>
                            <div style="position:relative; height:75px; width:100%; margin:0 auto;"><canvas id="grafPais"></canvas></div>
                        </div>
                    </div>
                ...
                <div class="exec-grid-2">
                    <div class="exec-box">
                        <h3 class="exec-title">Cursos Ministrados (${l.length})</h3>
                        ${L}
                    </div>
                    <div class="exec-box">
                        <h3 class="exec-title">Distribuição por Turmas (${c.length})</h3>
                        ${R}
                    </div>
                </div>

            </div>`,setTimeout(()=>{let e={responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},cutout:`72%`},t=[`#3b82f6`,`#8b5cf6`,`#f59e0b`,`#10b981`,`#f97316`,`#64748b`];Object.keys(T).length>0&&document.getElementById(`grafMensalidade`)&&new Chart(document.getElementById(`grafMensalidade`),{type:`doughnut`,data:{labels:Object.keys(T),datasets:[{data:Object.values(T),backgroundColor:t,borderWidth:0}]},options:e}),Object.keys(E).length>0&&document.getElementById(`grafVenda`)&&new Chart(document.getElementById(`grafVenda`),{type:`doughnut`,data:{labels:Object.keys(E),datasets:[{data:Object.values(E),backgroundColor:t,borderWidth:0}]},options:e}),Object.keys(P).length>0&&document.getElementById(`grafPais`)&&new Chart(document.getElementById(`grafPais`),{type:`bar`,data:{labels:Object.keys(P),datasets:[{data:Object.values(P),backgroundColor:`#475569`,borderRadius:2}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},scales:{x:{grid:{display:!1},ticks:{font:{size:6.5},maxRotation:0}},y:{beginAtZero:!0,border:{display:!1},ticks:{stepSize:1,font:{size:6.5}}}}}})},300)}catch{A.showToast(`Erro ao gerar dossiê.`,`error`)}finally{document.body.style.cursor=`default`}},A.gerarFichaSetup=async()=>{A.setTitulo(`Ficha de Matrícula`);let e=document.getElementById(`app-content`);e.innerHTML=`<p style="text-align:center;">Carregando...</p>`;try{let t=`
            <div style="display:flex; gap:10px; align-items:flex-end; flex-wrap:wrap;">
