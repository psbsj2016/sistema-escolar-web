...
`,m=async()=>{d.verificarNotificacoes(),d.setTitulo(`Visão Geral`);let e=document.getElementById(`app-content`);e.innerHTML=`<p style="padding:20px; text-align:center; color:#666;">Carregando painel de métricas...</p>`;try{let[t,n,r,i]=await Promise.all([d.api(`/alunos`),d.api(`/financeiro`),d.api(`/turmas`),d.api(`/cursos`)]),a=(Array.isArray(t)?t:[]).filter(e=>!e.status||e.status===`Ativo`),o=Array.isArray(n)?n:[],s=Array.isArray(r)?r:[],c=Array.isArray(i)?i:[],l=new Date,u=l.getMonth()+1,m=l.getFullYear(),h=a.map(e=>e.id),g=o.filter(e=>{if(!e.vencimento)return!1;let t=e.vencimento.split(`-`);return parseInt(t[1])===u&&parseInt(t[0])===m}),_=g.filter(e=>e.status===`Pago`).reduce((e,t)=>e+parseFloat(t.valor),0),v=g.filter(e=>e.status!==`Pago`&&h.includes(e.idAluno)).reduce((e,t)=>e+parseFloat(t.valor),0),y=o.filter(e=>e.status===`Pendente`&&new Date(e.vencimento+`T00:00:00`)<l&&h.includes(e.idAluno)).sort((e,t)=>new Date(e.vencimento)-new Date(t.vencimento)),b=e=>e.toLocaleString(`pt-BR`,{minimumFractionDigits:2,maximumFractionDigits:2}),x=d.usuario?d.usuario.tipo:`Gestor`,S=x!==`Professor`,C=y.length===0?`<div style="text-align:center; padding:20px; color:#27ae60; font-weight:bold; font-size:14px;">🎉 Excelente! Nenhum título em atraso.</div>`:y.map(e=>{let t=(a.find(t=>t.id===e.idAluno)||{}).whatsapp||``,n=e.vencimento.split(`-`).reverse().join(`/`),r=b(parseFloat(e.valor));return p(e.alunoNome||`Desconhecido`,n,r,t)}).join(``),w=JSON.parse(localStorage.getItem(d.getTenantKey(`escola_atalhos`)));(!w||!Array.isArray(w)||w.length===0)&&(w=[`novo_aluno`,`fin_carne`,`ped_chamada`,`ped_notas`,`ped_plan`,`ped_bol`]);let T=w.map(e=>{let t=(window.LISTA_FUNCIONALIDADES||[]).find(t=>t.id===e);return t&&t.roles.includes(x)?`<div class="shortcut-btn" onclick="${t.acao}"><div>${t.icon}</div><span>${t.nome}</span></div>`:``}).join(``);if(e.innerHTML=`
    ...
    `;if(a&&(a.innerHTML=s),o){o.innerText=`SIM, EXCLUIR 🗑️`,o.style.background=`#c0392b`;let r=o.cloneNode(!0);o.parentNode.replaceChild(r,o),r.onclick=async()=>{let i=r.innerText;r.innerText=`A excluir... ⏳`,r.disabled=!0,document.body.style.cursor=`wait`;try{let r=Array.from(t).map(e=>x.api(`/${n}/${e.value}`,`DELETE`));await Promise.all(r),x.showToast(`Registos excluídos com sucesso!`,`success`),x.fecharModal();let i=document.getElementById(`app-content`);i&&(i.innerHTML=`<p style="text-align:center; color:#666; padding:20px;">Atualizando lista... ⏳</p>`),await x.renderizarLista(e)}catch{x.showToast(`Erro ao processar exclusão.`,`error`),r.innerText=i,r.disabled=!1}finally{document.body.style.cursor=`default`,setTimeout(()=>{r&&(r.style.background=``)},500)}}}},window.App=window.App||{};var w=window.App;Object.assign(w,{renderizarWorkspaceAdmin:async()=>{w.setTitulo(`Gestão do Workspace`);let e=document.getElementById(`app-content`);e.innerHTML=`<p style="text-align:center; padding:40px; color:#666;">A carregar lista de alunos e acessos... ⏳</p>`;try{let[t,n]=await Promise.all([w.api(`/alunos`),w.api(`/usuarios`)]);w.workspaceCache={alunos:t.filter(e=>!e.status||e.status===`Ativo`),usuarios:n.filter(e=>e.tipo===`Aluno`||e.alunoRefId)},e.innerHTML=`
            ...
            `,w.filtrarWorkspaceAdmin()}catch{e.innerHTML=`<p style="color:#e74c3c; text-align:center; padding:40px;">Erro ao carregar a lista de alunos.</p>`}},filtrarWorkspaceAdmin:()=>{let e=(document.getElementById(`ws-busca-aluno`)?.value||``).toLowerCase(),t=document.getElementById(`ws-admin-tabela-container`);if(!t||!w.workspaceCache)return;let{alunos:n,usuarios:r}=w.workspaceCache,i=n.filter(t=>(t.nome||``).toLowerCase().includes(e));if(i.length===0){t.innerHTML=`<p style="text-align:center; padding:30px; color:#666;">Nenhum aluno encontrado com este nome.</p>`;return}let a=`
            <div class="table-responsive-wrapper">
                <table style="width:100%; text-align:left; border-collapse:collapse;">
                    <thead>
                    ...
                    <tbody>
        `;i.forEach(e=>{let t=r.find(t=>t.alunoRefId===e.id),n=t?`<span style="background:#eafaf1; color:#27ae60; padding:6px 12px; border-radius:6px; font-size:12px; font-weight:bold; border:1px solid #2ecc71;">✅ ${w.escapeHTML(t.login)}</span>`:`<span style="background:#fdf2f2; color:#e74c3c; padding:6px 12px; border-radius:6px; font-size:12px; font-weight:bold; border:1px solid #e74c3c;">❌ Sem Acesso</span>`,i=t?`<button class="btn-cancel" style="padding:8px 15px; font-size:12px; width:auto; font-weight:bold; color:#e74c3c; border-color:#e74c3c;" onclick="App.excluirAcessoWorkspace('${t.id}')">🗑️ Revogar Acesso</button>`:`<button class="btn-primary" style="padding:8px 15px; font-size:12px; width:auto; font-weight:bold; background:#8e44ad; border-color:#8e44ad;" onclick="App.abrirModalAcessoWorkspace('${e.id}', '${w.escapeJS(e.nome)}')">🔑 Gerar Acesso</button>`;a+=`
                <tr style="border-bottom:1px solid #f9f9f9; transition: background 0.2s;" onmouseover="this.style.background='#f4f6f7'" onmouseout="this.style.background='transparent'">
                    <td style="padding:15px; font-size:14px; font-weight:500; color:#333;">${w.escapeHTML(e.nome)}</td>
                    <td style="padding:15px;">${n}</td>
                    <td style="padding:15px; text-align:right;">${i}</td>
                </tr>
            ...
            <div style="background:#fff; padding:30px; border-radius:16px; max-width:400px; width:90%; box-shadow:0 20px 50px rgba(0,0,0,0.3); animation: slideIn 0.3s ease;">
                ...
                <p style="font-size:13px; color:#666; text-align:center; margin-bottom:25px;">Crie as credenciais para:<br><strong style="color:#3498db; font-size:15px;">${w.escapeHTML(t)}</strong></p>
