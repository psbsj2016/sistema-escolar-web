...
`,h=async()=>{f.verificarNotificacoes(),f.setTitulo(`Visão Geral`);let e=document.getElementById(`app-content`);try{let[t,n,r,i]=await Promise.all([f.api(`/alunos`),f.api(`/financeiro`),f.api(`/turmas`),f.api(`/cursos`)]),a=(Array.isArray(t)?t:[]).filter(e=>!e.status||e.status===`Ativo`),o=Array.isArray(n)?n:[],s=Array.isArray(r)?r:[],c=Array.isArray(i)?i:[],l=new Date,u=l.getMonth()+1,d=l.getFullYear(),h=a.map(e=>e.id),g=o.filter(e=>{if(!e.vencimento)return!1;let t=e.vencimento.split(`-`);return parseInt(t[1])===u&&parseInt(t[0])===d}),_=g.filter(e=>e.status===`Pago`).reduce((e,t)=>e+parseFloat(t.valor),0),v=g.filter(e=>e.status!==`Pago`&&h.includes(e.idAluno)).reduce((e,t)=>e+parseFloat(t.valor),0),y=o.filter(e=>e.status===`Pendente`&&new Date(e.vencimento+`T00:00:00`)<l&&h.includes(e.idAluno)).sort((e,t)=>new Date(e.vencimento)-new Date(t.vencimento)),b=e=>e.toLocaleString(`pt-BR`,{minimumFractionDigits:2,maximumFractionDigits:2}),x=f.usuario?f.usuario.tipo:`Gestor`,S=x!==`Professor`,C=y.length===0?`<div style="text-align:center; padding:20px; color:#27ae60; font-weight:bold; font-size:14px;">🎉 Excelente! Nenhum título em atraso.</div>`:y.map(e=>{let t=(a.find(t=>t.id===e.idAluno)||{}).whatsapp||``,n=e.vencimento.split(`-`).reverse().join(`/`),r=b(parseFloat(e.valor));return m(e.alunoNome||`Desconhecido`,n,r,t)}).join(``),w=JSON.parse(localStorage.getItem(f.getTenantKey(`escola_atalhos`)));(!w||!Array.isArray(w)||w.length===0)&&(w=[`novo_aluno`,`fin_carne`,`ped_chamada`,`ped_notas`,`ped_plan`,`ped_bol`]);let T=w.map(e=>{let t=(window.LISTA_FUNCIONALIDADES||[]).find(t=>t.id===e);return t&&t.roles.includes(x)?`<div class="shortcut-btn" onclick="${t.acao}"><div>${t.icon}</div><span>${t.nome}</span></div>`:``}).join(``);if(e.innerHTML=`
            ...
            <div class="shortcuts-grid">${T||`<p style="color:#666;">Nenhum atalho selecionado ou permitido.</p>`}</div>`,S){let e=document.getElementById(`graficoFinanceiro`);e&&(_>0||v>0)?new Chart(e,{type:`doughnut`,data:{labels:[`Recebido`,`Pendente`],datasets:[{data:[_,v],backgroundColor:[`#27ae60`,`#e74c3c`],borderWidth:0,hoverOffset:4}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},cutout:`75%`}}):e&&new Chart(e,{type:`doughnut`,data:{datasets:[{data:[1],backgroundColor:[`#eee`],borderWidth:0}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1},tooltip:{enabled:!1}},cutout:`75%`}})}}catch(t){console.error(t),e.innerHTML=`<p>Erro ao carregar dashboard.</p>`}},g=(e,t,n,r)=>{if(!f.verificarPermissao(`whatsapp`))return;if(!t||t.trim()===``||t===`undefined`){f.showToast(`Este aluno não tem um número de WhatsApp registado no sistema!`,`error`);return}let i=t.replace(/\D/g,``);t.trim().startsWith(`+`)?i=t.replace(/\D/g,``):(i.length===10||i.length===11)&&(i=`55`+i);let a=JSON.parse(localStorage.getItem(f.getTenantKey(`escola_perfil`)))||{},o=a.nome||`Nossa Instituição`,s=a.chavePix||`Não informada`,c=`🔔 *LEMBRETE DE VENCIMENTO*\nOlá, ${e}!\n\nConsta no nosso sistema que a sua mensalidade venceu no dia ${n}. Para realizar o pagamento de forma rápida, basta enviar o valor de *R$ ${r}* para a chave PIX abaixo:\n\n*Instituição:* ${o}\n*Banco:* ${a.banco||`Não informado`}\n*Chave PIX:* ${s}\n\n*Obs.:* _Após o pagamento, por favor, envie o comprovante por aqui para podermos dar baixa no sistema._\n\n🙏 Agradecemos desde já e desejamos-lhe um excelente dia! 😉✅`;window.open(`https://wa.me/${i}?text=${encodeURIComponent(c)}`,`_blank`)};window.App=window.App||{};var _=window.App;Object.assign(_,{verificarNotificacoes:async()=>{try{let e=_.usuario?_.usuario.tipo:`Gestor`,t=await _.api(`/sistema/notificacoes/nao-lidas`),n=await _.api(`/alunos`);await _.api(`/eventos`);let r=await _.api(`/financeiro`),i=await _.api(`/planejamentos`),a=await _.api(`/estoques`),o=await _.api(`/escola`),s=await _.api(`/cursos`).catch(()=>[]),c=await _.api(`/chamadas`).catch(()=>[]);if(Array.isArray(n)&&(n=n.filter(e=>!e.status||e.status===`Ativo`)),_.entidadeAtual===`aluno`&&Array.isArray(n)&&Array.isArray(_.listaCache)){let e=_.listaCache.map(e=>e.id);if(n.some(t=>!e.includes(t.id))){_.showToast(`Novo aluno recebido pela matrícula online.`,`success`),_.listaCache=n;let e=document.getElementById(`input-busca`);e&&(e.value=``),typeof _.filtrarTabelaReativa==`function`&&_.filtrarTabelaReativa()}}let l=[];Array.isArray(t)&&t.filter(e=>!e.lida).sort((e,t)=>new Date(t.dataCriacao||0)-new Date(e.dataCriacao||0)).slice(0,10).forEach(e=>{l.push({icon:e.tipo===`matricula_contrato`?`📝`:`🔔`,texto:`<b>${_.escapeHTML(e.titulo||`Nova notificação`)}</b><br>${_.escapeHTML(e.mensagem||``)}<br><small>Origem: ${_.escapeHTML(e.refLink||`Direto`)}</small>`,prioridade:1,acao:`App.renderizarContratos()`})});let u=new Date,d=u.getFullYear(),f=u.getMonth(),p=u.getDate(),m=`${d}-${String(f+1).padStart(2,`0`)}-${String(p).padStart(2,`0`)}`,h=u.getTime(),g=new Date(u);if(g.setDate(g.getDate()+1),`${g.getFullYear()}${String(g.getMonth()+1).padStart(2,`0`)}${String(g.getDate()).padStart(2,`0`)}`,o&&e===`Gestor`){let e=o.plano||`Teste`,t=0;if(o.dataExpiracao){let e=new Date(o.dataExpiracao).getTime();t=Math.ceil((e-h)/(1e3*60*60*24))}else{let n=o.dataCriacao?new Date(o.dataCriacao).getTime():h,r=Math.floor(Math.abs(h-n)/(1e3*60*60*24));t=(e===`Teste`?7:30)-r}e!==`Premium`&&e!==`Bloqueado`&&(t<=3&&t>0?l.push({icon:`⏳`,texto:`<b>Atenção Gestor:</b> O seu plano <b>${e}</b> expira em <b>${t} dia(s)</b>! Renove agora.`,acao:`App.renderizarTela('plano')`}):t<=0&&l.push({icon:`🚫`,texto:`<b>Urgente:</b> O seu acesso <b>expirou</b>! Regularize para continuar a usar o sistema.`,acao:`App.renderizarTela('plano')`}))}if(Array.isArray(n)&&n.forEach(e=>{e.nascimento&&e.nascimento.substring(5)===`${String(f+1).padStart(2,`0`)}-${String(p).padStart(2,`0`)}`&&l.push({icon:`🎂`,texto:`Hoje é o aniversário de <b>${_.escapeHTML(e.nome)}</b>! Clique para ver.`,acao:`App.renderizarLista('aluno')`})}),Array.isArray(n)&&n.forEach(e=>{e.dataMatricula&&e.dataMatricula.startsWith(m)&&l.push({icon:`🎉`,texto:`<b>Nova Matrícula!</b> O aluno <b>${_.escapeHTML(e.nome)}</b> foi registado hoje.`,acao:`App.renderizarContratos()`})}),e!==`Professor`&&Array.isArray(r)){let e=r.filter(e=>e.vencimento===m&&e.status===`Pendente`);e.length>0&&l.push({icon:`💲`,texto:`<b>Caixa de Hoje:</b> Existem <b>${e.length}</b> mensalidades a vencer no dia de hoje.`,acao:`App.renderizarTela('mensalidades')`})}if(e!==`Professor`){if(Array.isArray(r)&&Array.isArray(n)&&Array.isArray(i)&&Array.isArray(c)){let e=new Date(h-720*60*60*1e3);n.forEach(t=>{let n=i.find(e=>e.idAluno===t.id),a=null;if(r.forEach(e=>{e.idAluno===t.id&&e.status!==`Cancelado`&&(!e.idCarne||!e.idCarne.includes(`VENDA`))&&(!a||e.vencimento>a)&&(a=e.vencimento)}),a&&n){let r=a.split(`-`),i=new Date(r[0],r[1]-1,r[2]),o=new Date(d,f,p),u=i.getTime()-o.getTime(),m=Math.ceil(u/(1e3*3600*24));if(m<=30){let r=s.find(e=>e.id===t.cursoId||e.nome===t.planoCurso),i=r&&r.cargaHoraria?parseFloat(r.cargaHoraria):n.aulas?n.aulas.length*2:40,a=0,o=0,u=0,d=h;c.filter(e=>e.idAluno===t.id).forEach(t=>{if(!t.data)return;let n=t.data.includes(`/`)?t.data.split(`/`):t.data.split(`-`),r=new Date;r=t.data.includes(`/`)?new Date(n[2],n[1]-1,n[0],12,0,0):new Date(n[0],n[1]-1,n[2],12,0,0),isNaN(r.getTime())&&(r=new Date);let i=2;if(t.duracao&&t.duracao.includes(`:`)){let[e,n]=t.duracao.split(`:`).map(Number);i=e+(n||0)/60}else t.duracao&&(i=parseFloat(t.duracao)||2);d=Math.max(d,r.getTime()),(t.status===`Presença`||t.status===`Reposição`)&&(a+=i),r.getTime()>=e.getTime()&&r.getTime()<=h&&(o++,u+=i)});let f=i-a;if(f>0){let e=u>0?u:8,n=o>0?u/o:2,r=Math.ceil(f/n),s=f/e,c=new Date(d);c.setDate(c.getDate()+Math.ceil(s*30));let p=`${String(c.getDate()).padStart(2,`0`)}/${String(c.getMonth()+1).padStart(2,`0`)}/${c.getFullYear()}`,h=m<0?`já venceu há ${Math.abs(m)} dias`:m===0?`vence hoje`:`vence em ${m} dias`,g=i-a>0?`<br>Devido a faltas ou atrasos, cumpriu apenas <b>${parseFloat(a.toFixed(1))}h</b> das <b>${parseFloat(i.toFixed(1))}h</b> do curso.`:``;l.push({icon:`🎯`,texto:`<div style="line-height:1.5;"><b>Auditoria Preditiva (Mensalidade Extra):</b><br>
                                        A última mensalidade de <b style="color:#3498db;">${_.escapeHTML(t.nome)}</b> ${h}.${g}<br>
                                        Ainda restam <b>${parseFloat(f.toFixed(1))}h pendentes</b> (aprox. ${r} aulas). Ao ritmo atual (${parseFloat(e.toFixed(1))}h/mês), a conclusão será apenas em <b style="color:#e74c3c;">${p}</b>.<br>
                                        <i>Sugerimos gerar parcelas adicionais!</i></div>`,acao:`App.renderizarTela('mensalidades')`})}}}})}Array.isArray(a)&&a.forEach(e=>{let t=parseInt(e.quantidade)||0;t<=(parseInt(e.quantidadeMinima)||0)&&l.push({icon:`📦`,texto:`<b>Estoque Baixo:</b> Restam apenas ${t} unidades de <b>${_.escapeHTML(e.nome)}</b>!`,acao:`App.renderizarLista('estoque')`})})}let v=document.getElementById(`noti-badge`),y=document.getElementById(`noti-list`);if(l.length>0){v&&(v.innerText=l.length,v.style.display=`block`);let e=l.length>0?`
