...
class DragAndDropManager {
  ...
  prevPagePoint = new import_editor.Vec();
  intervalTimerId = -1;
  startDraggingShapes(movingShapes, point, cb) {
    ...
    for (const movingGroup of movingGroups) {
      const children = (0, import_editor.compact)(
  ...
  updateDraggingShapes(point, cb) {
    const { editor } = this;
    const draggingShapes = (0, import_editor.compact)(this.shapesToActuallyMove.map((s) => editor.getShape(s)));
    ...
    editor.run(() => {
      if (this.prevDraggingOverShape?.id === nextDraggingOverShape?.id) {
        if (cursorDidMove && nextDraggingOverShape && (0, import_editor.isShapeId)(nextDraggingOverShape.id) && !editor.inputs.getPreviousPagePoint().equals(currentPagePoint)) {
