...
var import_updateHoveredOverlayId = require("../../selection-logic/updateHoveredOverlayId");
...
class Idle extends import_editor.StateNode {
  ...
  onEnter() {
    ...
    if (!(0, import_updateHoveredOverlayId.updateHoveredOverlayId)(this.editor)) {
  ...
  onPointerMove() {
    ...
    if ((0, import_editor.unsafe__withoutCapture)(() => this.editor.getInstanceState()).isChangingStyle) {
      this.editor.updateInstanceState({ isChangingStyle: false });
    }
  }
  onPointerDown(info) {
    switch (info.target) {
      ...
      case "overlay": {
        ...
        const overlayType = overlay.props.overlayType;
      ...
      case "selection": {
        switch (info.handle) {
          ...
          default: {
            ...
            if (hoveredShape && !this.editor.getSelectedShapeIds().includes(hoveredShape.id) && (this.editor.options.selectLockedShapes || !hoveredShape.isLocked)) {
              this.onPointerDown({
                ...info,
                shape: hoveredShape,
                target: "shape"
  ...
  onRightClick(info) {
    switch (info.target) {
      case "canvas": {
        ...
        const hoveredShape = this.editor.getHoveredShape();
        const hitShape = hoveredShape && !this.editor.isShapeOfType(hoveredShape, "group") ? hoveredShape : this.editor.getShapeAtPoint(currentPagePoint, {
          margin: this.editor.getHitTestMargin(),
          hitInside: false,
          hitLabels: true,
          hitLocked: true,
