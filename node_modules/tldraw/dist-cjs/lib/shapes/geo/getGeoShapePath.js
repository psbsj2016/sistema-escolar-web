...
__export(getGeoShapePath_exports, {
  defaultGeoTypeDefinitions: () => defaultGeoTypeDefinitions,
  getGeoShapePath: () => getGeoShapePath,
  getGeoTypeDefinition: () => getGeoTypeDefinition
});
...
const defaultGeoTypeDefinitions = {
  rectangle: {
    snapType: "polygon",
    icon: "geo-rectangle",
    getPath(w, h, shape) {
...
function getGeoTypeDefinition(name, customGeoTypes) {
  return customGeoTypes?.[name] ?? defaultGeoTypeDefinitions[name];
}
const pathCache = new import_editor.WeakCache();
function getGeoShapePath(shape, strokeWidth, customGeoTypes) {
  return pathCache.get(shape, (s) => _getGeoPath(s, strokeWidth, customGeoTypes));
}
function _getGeoPath(shape, strokeWidth, customGeoTypes) {
  ...
  if (flipX || flipY) {
    return path.transform((0, import_flip.getFlipMatrix)({ flipX, flipY }, w, h));
  }
  return path;
}
