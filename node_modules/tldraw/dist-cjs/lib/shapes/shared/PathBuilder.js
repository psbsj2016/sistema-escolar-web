...
class PathBuilder {
  ...
  transform(mat) {
    ...
    for (const command of this.commands) {
      const [x, y] = import_editor.Mat.applyToXY(mat, command.x, command.y);
      switch (command.type) {
        case "move":
          next.commands.push({ ...command, x, y, _info: void 0 });
          break;
        case "line":
          next.commands.push({ ...command, x, y, _info: void 0 });
          break;
        case "cubic": {
          const [cp1x, cp1y] = import_editor.Mat.applyToXY(mat, command.cp1.x, command.cp1.y);
          const [cp2x, cp2y] = import_editor.Mat.applyToXY(mat, command.cp2.x, command.cp2.y);
          next.commands.push({
            ...command,
            x,
            y,
    ...
    for (let pass = 0; pass < passes; pass++) {
      ...
      for (const {
        ...
        if (command.isClose) {
          ...
          const direction = new import_editor.Vec(random(), random()).uni();
          const magnitude = Math.sqrt(Math.abs(random())) * offsetAmount;
          offset = import_editor.Vec.Mul(direction, magnitude);
        }
        if (command.type === "move") {
