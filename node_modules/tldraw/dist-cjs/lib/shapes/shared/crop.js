...
function getCropBox(shape, info, opts = {}) {
  ...
  const prevCropBox = new import_editor.Box(
    crop.topLeft.x * w,
    crop.topLeft.y * h,
  ...
  const newPoint = new import_editor.Vec(tempBox.x - crop.topLeft.x * w, tempBox.y - crop.topLeft.y * h).rot(shape.rotation).add(shape);
  return {
    id: shape.id,
    type: shape.type,
    x: newPoint.x,
...
function getCroppedImageDataForReplacedImage(imageShape, newImageWidth, newImageHeight) {
  ...
  const isOriginalCrop = (0, import_editor.isEqual)(imageShape.props.crop, defaultCrop);
  if (isOriginalCrop) {
