    ...
    const actionItems = [
      ...
      },
      {
        ...
        onSelect(source) {
          ...
          const frameId = (0, import_editor.createShapeId)();
          ...
          editor.run(() => {
            ...
            (0, import_frames.fitFrameToContent)(editor, frameId, { padding });
      ...
      },
      {
        ...
        onSelect(source) {
          ...
          trackEvent("remove-frame", { source });
          const selectedShapes = editor.getSelectedShapes();
          if (selectedShapes.length > 0 && selectedShapes.every((shape) => editor.isShapeFrameLike(shape))) {
            editor.markHistoryStoppingPoint("remove-frame");
            (0, import_frames.removeFrame)(
              editor,
              selectedShapes.map((shape) => shape.id)
      ...
      },
      {
        ...
        onSelect(source) {
          ...
          const point = pasteAtCursor ? editor.inputs.getCurrentPagePoint() : void 0;
          navigator.clipboard?.read().then((clipboardItems) => {
      ...
      },
      {
        ...
        onSelect() {
          const pasteAtCursor = !editor.user.getIsPasteAtCursorMode();
          const point = pasteAtCursor ? editor.inputs.getCurrentPagePoint() : editor.getViewportPageBounds().center;
          navigator.clipboard?.readText().then((text) => {
            if (text?.trim()) {
              editor.markHistoryStoppingPoint("paste");
              (0, import_defaultExternalContentHandlers.defaultHandleExternalTextContent)(editor, { text, point });
            }
          }).catch(() => {
            helpers.addToast({
              title: helpers.msg("action.paste-error-title"),
      ...
      },
      {
        ...
        onSelect: async (source) => {
          ...
          const screenPoint = editor.pageToScreen(new import_editor.Vec(centerX, centerY));
      ...
      },
      {
        ...
        onSelect: async (source) => {
          ...
          const a11yLive = (0, import_A11y.generateShapeAnnouncementMessage)({
            editor,
            selectedShapeIds,
            msg
          });
      ...
      },
      {
        ...
        onSelect: async (source) => {
          ...
          for (const mediaShape of mediaShapes) {
            ...
            if (!url) continue;
            const name = (asset.type === "video" || asset.type === "image") && !asset.props.src.startsWith("asset:") ? asset.props.name : "download";
            try {
              const resp = await (0, import_editor.fetch)(url);
              if (!resp.ok) throw new Error(`Failed to fetch asset: ${resp.status}`);
              const blob = await resp.blob();
              (0, import_exportAs.downloadFile)(
                new File([blob], name, { type: blob.type }),
                editor.getContainerDocument()
              );
            } catch {
              (0, import_editor.openWindow)(url, "_blank");
            }
          }
        ...
        async onSelect(source) {
          ...
          editor.run(() => {
            for (const style of editor.styleProps[shape.type].keys()) {
              ...
              if (value === void 0 || style === import_editor.GeoShapeGeoStyle) continue;
