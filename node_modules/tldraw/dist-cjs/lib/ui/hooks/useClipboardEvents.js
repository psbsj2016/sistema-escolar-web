...
__export(useClipboardEvents_exports, {
  extractIframeFromHtml: () => extractIframeFromHtml,
  handleNativeOrMenuCopy: () => handleNativeOrMenuCopy,
  handlePasteFromClipboardApi: () => handlePasteFromClipboardApi,
  handlePasteFromEventClipboardData: () => handlePasteFromEventClipboardData,
  isValidHttpURL: () => isValidHttpURL,
  putPastedExternalContent: () => import_putPastedContent2.putPastedExternalContent,
  resolvePasteModifiers: () => resolvePasteModifiers,
...
var import_putPastedContent = require("./clipboard/putPastedContent");
var import_putPastedContent2 = require("./clipboard/putPastedContent");
...
function stripHtml(html) {
  const doc = (0, import_editor.getGlobalDocument)().implementation.createHTMLDocument("");
...
function areShortcutsDisabled(editor) {
  return editor.menus.hasAnyOpenMenus() || (0, import_editor.activeElementShouldCaptureKeys)(false, editor.getContainerDocument());
}
const handleText = (editor, data, point, sources, clipboardPasteSource = "native-event") => {
  const validUrlList = getValidHttpURLList(data);
  if (validUrlList) {
    for (const url of validUrlList) {
      (0, import_pasteUrl.pasteUrl)(editor, url, point, sources, clipboardPasteSource);
    }
  } else if (isValidHttpURL(data)) {
    (0, import_pasteUrl.pasteUrl)(editor, data, point, sources, clipboardPasteSource);
  } else if (isSvgText(data)) {
    editor.markHistoryStoppingPoint("paste");
    (0, import_putPastedContent.putPastedExternalContent)(
      editor,
      {
        type: "svg-text",
        text: data,
...
async function handlePasteFromEventClipboardData(editor, clipboardData, point) {
      ...
      things.push({
        ...
        source: (async () => {
          for (const type of matchingTypes) {
            ...
            return import_editor.FileHelpers.rewriteMimeType(blob, (0, import_clipboard.getCanonicalClipboardReadType)(type));
          }
          return null;
        })()
      });
...
async function handleClipboardThings(editor, things, point, clipboardPasteSource) {
  const results = await Promise.all(
    things.filter((t) => t.type !== "file" && t.type !== "blob").map(
      (t) => new Promise((r) => {
        const thing = t;
  ...
  const files = things.filter((t) => t.type === "file" || t.type === "blob");
  if (files.length) {
    if (files.length > editor.options.maxFilesAtOnce) {
      throw Error("Too many files");
    }
    const fileBlobs = (0, import_editor.compact)(await Promise.all(files.map((t) => t.source)));
    return await (0, import_pasteFiles.pasteFiles)(editor, fileBlobs, point, results, clipboardPasteSource);
  }
  ...
  for (const result of results) {
    if (result.type === "text" && result.subtype === "url") {
      (0, import_pasteUrl.pasteUrl)(editor, result.data, point, results, clipboardPasteSource);
      return;
    }
  }
  for (const result of results) {
...
function useMenuClipboardEvents() {
  ...
  const copy = (0, import_react.useCallback)(
    async function onCopy(source) {
      (0, import_editor.assert)(editor, "editor is required for copy");
    ...
    const paste = (e) => {
      ...
      if (nativeShiftKey) {
        ...
        if (text?.trim()) {
          ...
          editor.markHistoryStoppingPoint("paste");
          (0, import_defaultExternalContentHandlers.defaultHandleExternalTextContent)(editor, { text, point: point2 });
          (0, import_editor.preventDefault)(e);
