...
async function defaultHandleExternalFileAsset(editor, { file, assetId }, options) {
  ...
  if (!sanitizedFile) (0, import_editor.assert)(false, "SVG file contained no safe content");
  const assetInfo = await getAssetInfo(editor, sanitizedFile, assetId);
  if (!assetInfo) (0, import_editor.assert)(false, `No asset util found for MIME type "${file.type}"`);
...
async function defaultHandleExternalFileReplaceContent(editor, { file, shapeId }, options) {
  const isSuccess = notifyIfFileNotAllowed(editor, file, options);
  if (!isSuccess) (0, import_editor.assert)(false, "File checks failed");
  const sanitizedFile = await maybeSanitizeSvgFile(file);
  ...
  const hash = (0, import_editor.getHashForBuffer)(await sanitizedFile.arrayBuffer());
  const assetId = import_editor.AssetRecordType.createId(hash);
  editor.createTemporaryAssetPreview(assetId, sanitizedFile);
  const assetInfoPartial = await getAssetInfo(editor, sanitizedFile, assetId);
...
async function defaultHandleExternalTextContent(editor, { point, text, html }) {
  ...
  let autoSize;
  let align = "middle";
  const htmlToMeasure = (0, import_richText.renderHtmlFromRichTextForMeasurement)(editor, richTextToPaste);
  ...
  const rawSize = editor.textMeasure.measureHtml(htmlToMeasure, {
    ...
    fontFamily: (0, import_default_shape_constants.getFontFamily)(theme, defaultProps.font),
    fontSize: theme.fontSize * import_default_shape_constants.FONT_SIZES[defaultProps.size],
    maxWidth: null
  });
...
function notifyIfFileNotAllowed(editor, file, options) {
  const {
  ...
  const isFileTypeAllowed = !!editor.getAssetUtilForMimeType(file.type) || (acceptedImageMimeTypes?.includes(file.type) ?? false) || (acceptedVideoMimeTypes?.includes(file.type) ?? false);
...
async function getAssetInfo(editor, file, assetId) {
  if (!assetId) {
