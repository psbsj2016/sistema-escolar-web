...
.tl-container {
	 ...
	 *
	 * collaborators, watermark, and canvas-in-front are siblings of .tl-canvas
	 * (outside it), so they compete with the UI layer (.tlui-layout at z-index 300).
	 * canvas-in-front must be below the UI layer (300) to sit between
	 * the canvas and the UI; collaborator cursors sit above canvas content
	 * (and layers that anchor to it, like comment pins) but below the
	 * in-front content and the UI panels. The watermark (a licensing element)
	 * sits above that content so it can't be occluded, but still below the
	 * in-front content and the UI.
	 */
	--tl-layer-canvas-hidden: -999999;
	--tl-layer-canvas-background: 100;
	--tl-layer-canvas-grid: 150;
	--tl-layer-collaborators: 245;
	--tl-layer-watermark: 248;
...
/* The collaborator cursor layer: a sibling of .tl-canvas, above all canvas content, below the
   in-front layer (250) and the UI panels (300). Display-only — pointer events pass through. */
.tl-collaborators {
	position: absolute;
	inset: 0px;
	z-index: var(--tl-layer-collaborators);
	pointer-events: none;
	overflow: clip;
	contain: layout style size;
}

.tl-collaborators__item {
	position: absolute;
	top: 0px;
	left: 0px;
	overflow: visible;
