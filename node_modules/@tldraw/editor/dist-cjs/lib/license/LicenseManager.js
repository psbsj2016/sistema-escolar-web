...
class LicenseManager {
  ...
  featureFlags = (0, import_state.atom)("license feature flags", {
    ...NO_FEATURES
  });
  verbose = true;
  ...
  getIsDevelopment() {
    ...
    return protocol === "http:" || protocol === "https:" && this.isLoopbackHost(hostname) || process.env.NODE_ENV !== "production";
  }
  ...
  async getLicenseFromKey(licenseKey) {
    ...
    try {
      ...
      const isCommentingEnabled = isCollaborationEnabled || this.isFlagEnabled(licenseInfo.flags, FLAGS.FEAT_COMMENTING);
  ...
  getExpirationDateWithoutGracePeriod(expiryDate) {
    return new Date(
      Date.UTC(
        ...
        // Add 1 day so the named date is fully usable
      )
    );
  }
  getExpirationDateWithGracePeriod(expiryDate) {
    return new Date(
      Date.UTC(
        expiryDate.getUTCFullYear(),
        expiryDate.getUTCMonth(),
        expiryDate.getUTCDate() + GRACE_PERIOD_DAYS + 1
...
function getEnabledFeatures(result, licenseState, isDevelopment) {
  if (isDevelopment) {
    return { ...ALL_FEATURES };
  }
  if (!result.isLicenseParseable || licenseState !== "licensed" && licenseState !== "licensed-with-watermark") {
    return { ...NO_FEATURES };
  }
  if (result.isEvaluationLicense) {
    return { ...ALL_FEATURES };
