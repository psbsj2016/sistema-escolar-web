...
class Editor extends EventEmitter {
  ...
  getAttributionDisplayName(userId) {
    ...
    return this.store.props.users.resolve(userId).get()?.name ?? this.store.get(createUserId(userId))?.name ?? null;
  }
  /**
  ...
  getAttributionUser(userId) {
    if (!userId) return null;
    return this.store.props.users.resolve(userId).get() ?? this.store.get(createUserId(userId)) ?? null;
  }
  /**
  ...
  putContentOntoCurrentPage(content, opts = {}) {
    ...
    if (point) {
      ...
      for (const shape of selectedShapes) {
        const candidate = canAcceptAll(shape) ? shape : this.findShapeAncestor(shape, canAcceptAll) ?? (isShapeId(shape.parentId) ? this.getShape(shape.parentId) : null);
        if (!candidate) {
          selectedParent = null;
          break;
        }
  ...
  _flushEventForTick(info) {
    ...
    if (info.type === "pointer") {
      ...
      if (info.name === "right_click" && this.getCurrentToolId() !== "select" && this.getStateDescendant("select")) {
