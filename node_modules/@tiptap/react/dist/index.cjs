...
var EditorInstanceManager = class EditorInstanceManager {
	...
	*/
	createEditor() {
		...
		return new _tiptap_core.Editor(optionsToApply);
	}
	/**
	* Get the current editor instance.
	*/
...
//#endregion
//#region src/ReactNodeViewSelectionManager.ts
/**
* One selection listener per editor, instead of one per node view.
*/
var ReactNodeViewSelectionManager = class {
	constructor(editor) {
		...
		this.handleSelectionUpdate = () => {
			...
			this.rafId = requestAnimationFrame(() => {
				this.rafId = null;
				for (const view of this.views) view.checkSelection();
			});
		};
		this.editor = editor;
	}
	register(view) {
		if (this.views.size === 0) this.editor.on("selectionUpdate", this.handleSelectionUpdate);
		this.views.add(view);
	}
	unregister(view) {
...
function getReactNodeViewSelectionManager(editor) {
